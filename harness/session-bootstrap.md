...
- **Note (mechanism, only on an UPDATE AVAILABLE warning from the SessionStart hook):** the hook warning is a nonblocking notification and may supply the named loaded/marketplace commits, but the bootstrap helper still owns the closed update result and exact security-policy disposition.
