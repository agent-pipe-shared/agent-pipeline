...
export function validateLicenseContract(root) {
  ...
  requireText(root, "specs/2026-07-19-sprint-sentinel-epic/snt-1-activation-prerequisite.md", findings, [["explicit HAW-E activation blocker", /blocked; no HAW-E Result intent, release consent, publication, or\s+backlog mutation is authorized/u], ["missing dual license-gate digests", /private license-gate receipt digest.*neutral-public license-gate receipt digest/su], ["no fabricated ledger evidence", /append-only history remains truthful.*must not be edited.*invented values/su], ["sanctioned writer path", /applyBacklogTransition.*recoverable transaction writer/su], ["closed-state writer limitation", /already `closed`.*no same-state evidence-amendment\s+operation/su], ["external prerequisite digest set", /`resultSha256`.*`transitionSha256`.*`privateLicenseGateSha256`.*`neutralPublicLicenseGateSha256`/su]]);
