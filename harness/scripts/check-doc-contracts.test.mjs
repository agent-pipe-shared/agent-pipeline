...
import {
  MANIFEST_PATHS,
  checkEnforcementDocument,
  compareEnforcementRegistrations,
  collectEnforcementRegistrations,
  escapeMarkdownCell,
...
function enforcementFixture() {
  const { root } = fixture();
  for (const path of Object.values(MANIFEST_PATHS)) write(root, path, readFileSync(join(REPO, path), "utf8"));
  write(root, "docs/enforcement.md", renderEnforcementDocument({ rootDir: root }));
  return {
    root,
    trackedPaths: [
...
test("generated enforcement document derives the three runner schemas without classifying guards", () => {
  const registration = collectEnforcementRegistrations();
  assert.deepEqual(
    registration.sources.map(({ runner, path }) => ({ runner, path })),
    [
      { runner: "Claude Code", path: MANIFEST_PATHS["Claude Code"] },
      { runner: "Codex", path: MANIFEST_PATHS.Codex },
      { runner: "Antigravity", path: MANIFEST_PATHS.Antigravity },
    ],
  );
  assert(registration.rows.some((row) => row.runner === "Codex" && row.event === "SubagentStart" && row.matcher === "(all)"
    && row.command.endsWith('codex-slicing-hint.mjs" SubagentStart')));
  assert(registration.rows.some((row) => row.runner === "Antigravity" && row.event === "PreInvocation"
    && row.command === "node hooks/antigravity-slicing-hint.mjs deliver"));
  assert(registration.rows.some((row) => row.runner === "Claude Code" && row.event === "PreToolUse"
    && row.command.includes("guard-slicing.mjs")));

  const document = renderEnforcementDocument();
  assert.match(document, /## Maintained explanation and limits/u);
  assert.match(document, /expressly advisory/u);
  assert.match(document, /does not say that the command blocks/u);
  assert.doesNotMatch(document, /every registered command blocks/u);
  assert.doesNotMatch(document, /Claude Code row is coverage for Codex/u);
});

test("generated enforcement registration order uses UTF-8 bytes, not the host locale", () => {
  const rows = ["z", "~", "é", "😀", "!"].map((command) => ({
    runner: "same",
    event: "same",
    matcher: "same",
    command,
  ...
  assert.deepEqual(
    rows.sort(compareEnforcementRegistrations).map((row) => row.command),
    ["!", "z", "~", "é", "😀"],
  );
});

test("generated enforcement document changes with a manifest and rejects malformed registrations", () => {
  const { root } = enforcementFixture();
  const baseline = renderEnforcementDocument({ rootDir: root });
  const codexPath = join(root, MANIFEST_PATHS.Codex);
  const codex = JSON.parse(readFileSync(codexPath, "utf8"));
  codex.hooks.PreToolUse[0].matcher = "changed|native-matcher";
  writeFileSync(codexPath, JSON.stringify(codex, null, 2) + "\n");
  assert.notEqual(renderEnforcementDocument({ rootDir: root }), baseline, "a changed registration must change the generated bytes");

  codex.hooks.PreToolUse[0].hooks[0].command = "";
  writeFileSync(codexPath, JSON.stringify(codex, null, 2) + "\n");
  assert.throws(
    () => renderEnforcementDocument({ rootDir: root }),
    /codex-hooks\.json\.hooks\.PreToolUse\[0\]\.hooks\[0\]\.command: expected a non-empty string/u,
  );
});

test("manifest table cells encode pipes, backticks, newlines, and HTML-looking input", () => {
  const special = "node a|b" + String.fromCharCode(96) + "c\n<script>&";
  assert.equal(
    escapeMarkdownCell(special),
    "node a&#124;b&#96;c&#10;&lt;script&gt;&amp;",
  );
  const { root } = enforcementFixture();
  const codexPath = join(root, MANIFEST_PATHS.Codex);
  const codex = JSON.parse(readFileSync(codexPath, "utf8"));
  codex.hooks.PreToolUse[0].hooks[0].command = special;
  writeFileSync(codexPath, JSON.stringify(codex, null, 2) + "\n");
  const rendered = renderEnforcementDocument({ rootDir: root });
  assert.match(rendered, /node a&#124;b&#96;c&#10;&lt;script&gt;&amp;/u);
  assert.doesNotMatch(rendered, /<script>/u);
});

test("a stale tracked enforcement page fails the existing documentation contract", () => {
  const { root, trackedPaths } = enforcementFixture();
  assert.equal(checkEnforcementDocument({ rootDir: root }).ok, true);
  write(root, "docs/enforcement.md", "# stale\n");
  const result = checkRepository(root, {
    trackedPaths,
    markdownPaths: trackedPaths.filter((path) => path.endsWith(".md")),
  });
  assert.equal(result.stats.enforcementDocument, "checked");
  assert(
    result.findings.some((finding) => finding.startsWith("generated-enforcement: docs/enforcement.md: document bytes differ")),
    "expected generated-document drift finding; got: " + result.findings.join(" | "),
  );
});

test("source manifest identity keeps a deleted enforcement page fail-closed", () => {
  const { root, trackedPaths } = enforcementFixture();
  const withoutPage = trackedPaths.filter((path) => path !== "docs/enforcement.md");
  const result = checkRepository(root, {
    trackedPaths: withoutPage,
    markdownPaths: withoutPage.filter((path) => path.endsWith(".md")),
  });
  assert.equal(result.stats.enforcementDocument, "checked");
  assert(
    result.findings.includes(
      "generated-enforcement: docs/enforcement.md: required generated page is not tracked -- run: node harness/scripts/generate-enforcement-doc.mjs --write and add the page",
    ),
  );
});

test("a tracked enforcement page stays checked when one source manifest is removed", () => {
  const { root, trackedPaths } = enforcementFixture();
  const removedManifest = MANIFEST_PATHS.Codex;
  rmSync(join(root, removedManifest));
  const result = checkRepository(root, {
    trackedPaths: trackedPaths.filter((path) => path !== removedManifest),
    markdownPaths: trackedPaths.filter((path) => path.endsWith(".md")),
  });
  assert.equal(result.stats.enforcementDocument, "checked");
  assert(
    result.findings.some((finding) => finding.startsWith("generated-enforcement: docs/enforcement.md: generation failed: plugins/pipeline-core/hooks/codex-hooks.json: cannot read manifest")),
    "expected missing-manifest contract finding; got: " + result.findings.join(" | "),
  );
});

test("enforcement contract reports malformed source manifests", () => {
  const { root, trackedPaths } = enforcementFixture();
  const codexPath = join(root, MANIFEST_PATHS.Codex);
  ...
  assert(
    result.findings.some((finding) => finding.startsWith("generated-enforcement: docs/enforcement.md: generation failed: plugins/pipeline-core/hooks/codex-hooks.json: invalid JSON")),
    "expected malformed-manifest contract finding; got: " + result.findings.join(" | "),
  );
});

test("enforcement generator CLI rejects unknown, duplicate, and contradictory flags without writing", () => {
  assert.throws(() => parseEnforcementCliArgs([]), /exactly one/u);
  assert.throws(() => parseEnforcementCliArgs(["--write", "--check"]), /exactly one/u);
  assert.throws(() => parseEnforcementCliArgs(["--stdout", "--unknown"]), /unknown argument/u);
  assert.throws(() => parseEnforcementCliArgs(["--stdout", "--root", "--check"]), /--root requires/u);
  assert.throws(() => parseEnforcementCliArgs(["--stdout", "--root", "one", "--root", "two"]), /--root requires/u);

  const { root } = fixture();
  for (const path of Object.values(MANIFEST_PATHS)) write(root, path, readFileSync(join(REPO, path), "utf8"));
  const output = join(root, "docs", "enforcement.md");
  const result = runEnforcementCli(root, ["--stdout", "--check"]);
  assert.equal(result.status, 2);
  assert.match(result.stderr, /exactly one/u);
  assert.equal(existsSync(output), false, "invalid CLI input must not create the generated document");
});
