...
**Skip-decision logging (`pipeline.critic-skip-decision-logged`):** When the trigger-matrix evaluation above determines a Critic is NOT required for a dispatch (rows T0/T5, or any row a given diff falls out of entirely), that determination MUST be logged as a checkable record, not left as a silent absence of Critic evidence — a repository with zero Critic artifacts must be distinguishable, from repository state alone, between "zero because none were ever required" and "zero despite N required". The smallest fit is a field on the existing dispatch record (`evidence/dispatch-record-<TASK_ID>.json`), not a new ledger file: an OPTIONAL `criticSkip` object (`{"schema": "pipeline.critic-skip-decision.v1", "reason": "<optional>"}`) naming, at minimum, that a skip determination was made for that task id — the record's own `taskId` field already answers "which work package". A reason string is a plus, never mandatory. Forward-looking only: a dispatch record from before this rule landed is not retroactively required to carry the field, and a check built against this convention must not flag one for its absence (`plugins/pipeline-core/lib/critic-skip-decision.mjs`; `backlog/items/2026-08-29-critic-skip-not-an-explicit-logged-decision.md`).
