...
**Critic-decision logging (`pipeline.critic-skip-decision-logged`):** Evaluate the trigger matrix before opening a v3 dispatch record. Persist the closed trigger input (`rigorLevel`, `riskClass`, `riskFlag`, and the mechanical/architecture/guardrails/security diff flags) plus its deterministically evaluated `appliedRow`. Rows T0/T5 use `criticSkip`; rows T1-T4 use `criticRequired`. A required decision remains an explicit Verify finding until the coordinator replaces it with `criticEvidence` bound to the same task, candidate commit, repository-relative artifact path, and artifact digest. Exactly one of these three dispositions is allowed. A free-text reason never selects a row and cannot turn mandatory work into T5. Coverage and authorship checks independently derive conservative A/G/S signals from the record's actual commit paths and reject any under-declaration; T0 is admitted only when every changed path belongs to the bounded mechanical/generated/lockfile set. Pre-v3 records remain readable historical evidence but cannot bind a newly verified delivery. This is forward-looking without silently accepting a newly written legacy record (`plugins/pipeline-core/lib/critic-skip-decision.mjs`; `backlog/items/2026-08-29-critic-skip-not-an-explicit-logged-decision.md`).
