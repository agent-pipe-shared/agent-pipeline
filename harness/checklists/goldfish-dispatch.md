...
- [ ] Writing task → worktree per project calibration; workflow/runner dispatch → provider-neutral P5B runner → P5 structured MP-10 preflight before exactly one injected synthetic-adapter call: declared mode + matching side effects, canonical nonempty paths, calibrated isolation, active guard, task-tight allowlist, exact verify and escalation target. `bounded-write` only with positive calibrated capability evidence; `isolated-write` requires hooks + allowlist + isolated worktree. Ordinary local/serial writers remain worktree-optional.
