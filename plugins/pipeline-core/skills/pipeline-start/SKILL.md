...
- **`migration-required` (V0/V1/V2):** stop normal bootstrap with no
  ...
  authority.
- **`host-layout-incompatible`:** STOP with no write and no confirmation.
  Report the conflicting capability and the exact remedy returned by `inspect`.
  Never remove, overwrite, ignore, or silently relocate environment-owned
  control paths; a supported host integration or writable requested root is
  required.
