...
6. Reject secret-like material and raw prompts, chat history, or raw logs. Redact usernames, hostnames, email addresses, home paths, private remotes, and private network coordinates. Reject every URL or repository remote in free-text fields; repository references are allowed only in `sourceBacklogLinks`, where each canonical GitHub URL must match the resolved public target. Never render a rejected coordinate in a preview. Do not retain rejected or pre-sanitized input.
