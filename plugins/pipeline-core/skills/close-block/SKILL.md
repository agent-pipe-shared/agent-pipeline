...
4. **Drift checks ("Doku ist Snapshot, Code ist Wahrheit"):**
...
7b. **Error-register update (mandatory, after Learn + measure, before Telemetry)** — this sub-step does not renumber the surrounding steps (same contract as 6b): read `backlog/error-register.md` (project's error-register, if the project has adopted one — the agent-pipeline repo's own copy is the canonical reference for the mechanics below). For every distinct error/friction class surfaced this block (Goldfish stop conditions, Critic findings ≥ minor, harness/tooling friction, Elephant process misses):
   - **New class** → append one row (Fehlerklasse · Kategorie · zuerst gesehen = today · zuletzt gesehen = today · Status = `neu`). Merge into an existing similar row instead of adding a near-duplicate (semantic consolidation, capped board, ~30 rows) — never a growing single-incident list.
   - **Recurring class** (same root cause, already listed) → update `zuletzt gesehen` to today; flip Status to `WIEDERHOLT → Triage-Pflicht` if this is genuinely the second occurrence of the SAME root cause (a class already `disponiert` by a working mechanism does not flip back to `WIEDERHOLT` merely because a superficially similar symptom occurred once more with a different root cause — note the reasoning in the row if this judgment call comes up).
   - **Every `WIEDERHOLT` row gets a disposition or an explicit, reasoned deferral note THIS SAME close** — hierarchy Mechanismus (hook/guard/script) > Template (briefing/checklist line) > kuratierte Lektion (retro entry/prose rule), never left bare.
...
8. **Telemetry line (MP-20), auto-captured via `usage-ledger.mjs`:** `/usage` is a **user command and session-scoped** — numbers are NOT retrievable after the session ends, which is why the token half of this step now runs through `harness/scripts/usage-ledger.mjs` instead of a manual paste. New order of operations:
   ...
   2. **Append the row** to the project's `telemetry/costs.md` (create the file with the header below if absent) — token half = **erhoben (Skript)**, $-half = **geschätzt** (marked with the `asOf` date of `harness/scripts/model-prices.json`); the advisor-model $ estimate is additionally called out on its own in „Besonderheiten" (the real-money share when a paid external model is configured). As before, **one row per session/block AND one row per goldfish dispatch** of this block.
   3. **`/usage` paste stays OPTIONAL**, asked once as before (EL-17c) — now framed as „Limits/Abgleich" (limit standings %, credit %), no longer the data source for the token column. Decline/absence is fine; the row still exists — this replaces „nicht erhoben" as the default outcome. „nicht erhoben" remains reserved for actual script failure (5. below).
   4. **Standing Nachtrag rule:** a PO `/usage` paste arriving at ANY later time (any session of this repo) is worked into the NAMED session's existing row as a dated Nachtrag — real numbers OVERRIDE the estimate, the estimate stays visible in parentheses (calibration data for the estimator, MP-21); accompanying PO comments land in „Besonderheiten".
   5. **Fail-open:** script failure (missing/unreadable transcripts root, malformed price file, etc.) → record „nicht erhoben (Skript-Fehler: <kurz>)" in the token column and continue the ritual — telemetry must never block a close.

   | Datum | Session/Block | Rolle | Modell/Effort | Aufgabe (kurz) | Tokens laut `/usage` | First-Pass (j/n) | Eingriffe nötig (j/n) | Besonderheiten |
   |---|---|---|---|---|---|---|---|---|

   Conventions (MP-20): Rolle ∈ {Elephant, Goldfish, Critic, Workflow}; First-Pass/Eingriffe are maintained per goldfish dispatch, Elephant/Critic rows carry „—"; „Besonderheiten" = escalations (MP-05/MP-07), advisor-model fallback / advisor-model $ share, cache anomalies, calibration runs, workflow agent count, Nachtrag entries (see 4. above); headless/`--bare` runs add `total_cost_usd` (marked as $) in the token column. `/context` reading may be noted here too. **Long sessions:** append the telemetry row per completed dispatch wave DURING the session, not only at the close — close-anchored-only persistence loses `/usage` data irrecoverably on a crash.
...
2. **Authorship check:** the same mandatory question as full-ritual step 6b — „Von wem stammen die
   Produktions-Diffs dieser Session?" — a single-package close still gets this; the incident class it
