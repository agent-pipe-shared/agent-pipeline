...
4. **Drift checks ("Docs are a snapshot, code is truth"):**
   ...
   - **Session-owned residue:** when continuity runtime carries a `sessionCleanup` handle, run `session-cleanup.mjs cleanup --repo "$PWD" --session-descriptor <sessionId> --expected-descriptor-sha256 <persisted-digest>` and then its read-only `hygiene` command with the same bound handle. A failed cleanup, missing or changed descriptor, remaining manifest or noncanonical linked worktree makes the close unfinished; never infer deletion targets or print the descriptor path/nonce.
