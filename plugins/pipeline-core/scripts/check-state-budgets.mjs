...
/** Per-root, because the machine State head follows the authority tier (ADR-0054). */
export function canonicalNormalBootstrapPaths(root = DEFAULT_ROOT) {
  return Object.freeze([CANONICAL_HUMAN_STATE_PATH, canonicalMachineStatePath(root)]);
}
...
export function checkStateBudgets(root = DEFAULT_ROOT, resultPath) {
  ...
  const canonicalBootstrapPaths = canonicalNormalBootstrapPaths(root);
  if (bootstrapPaths.length !== canonicalBootstrapPaths.length
    || !canonicalBootstrapPaths.every((path, index) => bootstrapPaths[index] === path)) {
    findings.push("budgets.bootstrapPaths must equal the canonical normal bootstrap set");
  }
  if (status && (status.humanStatePath !== CANONICAL_HUMAN_STATE_PATH || status.machineStatePath !== machineStatePath)) {
  ...
  const machine = byteSize(root, machineStatePath, findings, "canonical machine state head");
  const bootstrap = canonicalBootstrapPaths.map((path, index) => byteSize(root, path, findings, `canonicalBootstrapPaths[${index}]`)).filter(Boolean);
