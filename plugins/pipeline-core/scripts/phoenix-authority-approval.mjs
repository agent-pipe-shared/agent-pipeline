...
const usage = "Usage: phoenix-authority-approval.mjs prepare|approve|verify --repo-root <repo> --directory <external-dir> --proposal <repo-file> (approve must run in an interactive terminal — it prompts for the private key passphrase)";
...
function run(argv = process.argv.slice(2)) {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) throw new Error("approve requires an interactive terminal (no TTY detected) — openssl needs to prompt for the private key passphrase");
