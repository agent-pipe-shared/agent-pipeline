...
import { GOVERNANCE_FORK_DISPOSITION_APPROVAL, governanceForkDispositionApprovalSubject, readLocalRepositoryFingerprint } from "../lib/governance-event-store.mjs";
...
const capturePolicy = { schema: "pipeline.governance-capture-policy.v1", policyId: "fixture", revision: "e".repeat(64), defaultAction: "deny", streams: ["human", "agent", "lifecycle"].map((origin) => ({ origin, purpose: origin === "human" ? "authority-history" : origin === "agent" ? "declared-assumption" : "deterministic-lifecycle", materiality: "required", personalIdentifiability: "prohibited", contextualIdentifiability: "prohibited", storageProfile: "repository-public-safe", retention: "repository-retained", disclosure: "repository-visible", encryptionGeneration: null })), sanitizedReceipt: { allowEventId: true, allowEventDigest: true, allowCheckpoint: true, allowReasonText: false }, mandatoryEventClasses: [] };
function intent() { return { schema: "pipeline.governance-event-envelope.v1", payloadSchema: "pipeline.lifecycle-governance-event.v1", canonicalization: "RFC8785", digestAlgorithm: "sha-256", eventId: "cli-event", idempotencyKey: "cli-idem", origin: "lifecycle", authorityClass: "non-authoritative", eventType: "lifecycle.dispatch", occurredAtEpochMs: 1, observedAtEpochMs: 1, timeAssurance: "locally-observed", repositoryFingerprint: fingerprint, sourceUri: `urn:pipeline:repository:${fingerprint}`, streamId: "lifecycle", correlation: { featureId: absent, packageId: "phoenix-3", requestId: absent, sessionId: absent, dispatchId: "dispatch-1", traceId: absent }, candidate, artifacts: [absent], policy: { policyDigest: absent, configurationDigest: absent, capturePolicyDigest, redactionPolicyDigest: absent }, classification: "repository-public-safe", storageProfile: "repository-public-safe", retentionCompatibility: "repository-retained", disclosureClass: "repository-visible", payload: { eventId: "lifecycle-cli-1", kind: "dispatch", status: "active", reasonCode: "DISPATCHED", correlation: { packageId: "phoenix-3", dispatchId: "dispatch-1", attemptId: "attempt-1", workerId: "worker-1", correlationId: "correlation-1", queueRevision: 0 }, candidate, invalidatesEventId: null, supersedesEventId: null } }; }
// NVA-REPOID-3: the store's bound identity, not the legacy path-derived hash
// -- this fixture has no filename-matching need pinning it to
// derivePoGateRepositoryFingerprint.
async function fixture() { const root = await mkdtemp(path.join(os.tmpdir(), "governance-event-cli-")); execFileSync("git", ["init", "-q", root]); fingerprint = await readLocalRepositoryFingerprint({ repositoryRoot: root }); registry.repositoryFingerprint = fingerprint; capturePolicyDigest = canonicalSha256(capturePolicy); await mkdir(path.join(root, "governance/events"), { recursive: true }); await writeFile(path.join(root, "governance/events/registry.json"), `${canonicalizeJson(registry)}\n`); await writeFile(path.join(root, "governance/events/capture-policy.json"), `${canonicalizeJson(capturePolicy)}\n`); return root; }
