...
const NORMAL_TRIGGER_ROWS = new Set(["T2", "T3", "T4", "T6"]);
...
/**
 * Explicit affected-Codex Critic call site.  The host must pass a selected
 * transport ID; the generic bridge performs exact readback before the first
 * child and returns the sanitized execution receipt binding.
 */
export async function runSelectedCodexCritic({ selectionId, requested, references = [] }, transport) {
  return runSandboxedReadonlyHostBridge({ selectionId, duty: "critic", requested, references }, transport);
}
    ...
    try {
      dependencies = createCodexSandboxRuntimeTransport({
        sandboxContext: {
          ...
          referenceSetSha256: request.dispatch?.referenceSetSha256,
        },
        sandboxRuntime,
        hostBridge,
      });
...
function selectedCriticHostBridge(iterator) {
  const receive = async (requestId) => {
    ...
    let response;
    try { response = JSON.parse(next.value); } catch { fail("selected Critic host result is not JSON"); }
    exactKeys(response, ["schema", "type", "requestId", "execution"], "selected Critic host result");
    if (response.schema !== "pipeline.codex-critic-selected-host.v1" || response.type !== "execution.result" || response.requestId !== requestId) {
      fail("selected Critic host result does not match its launch request");
    }
  ...
  return {
    async launch(request) {
      ...
      const execution = await receive(requestId);
      return { childStarted: execution?.terminal?.childStarted === true, execution };
    },
...
function assuranceForRequest(request) {
  if (request.trigger_row === "T1") return T1_ASSURANCE;
  if (NORMAL_TRIGGER_ROWS.has(request.trigger_row)) return ASSURANCE;
  fail("request trigger row cannot select an assurance");
}
...
/**
 * Read only the exact optional authorization shape emitted before the standing
 * T1 route existed.  This is compatibility metadata, never an authorization
 * input: assuranceForRequest deliberately ignores it.
 */
function validateLegacyDispatchAuthorization(authorization, candidateCommit) {
  ...
  if (authorization.kind !== "named-po-waiver" || authorization.authority !== "PO"
    ...
    || authorization.candidate_commit !== candidateCommit) {
    fail("legacy normal lane authorization is invalid or stale");
  }
}

function validateLegacyReceiptAuthorization(receipt) {
  const authorization = receipt.normalLaneAuthorization;
  if (authorization === undefined) return;
  if (authorization.kind !== "named-po-waiver" || authorization.authority !== "PO"
    || !SAFE_ID.test(authorization.riskId) || !SAFE_ID.test(authorization.scope)
    || !SHA256.test(authorization.evidenceSha256)
    || authorization.candidateCommit !== receipt.candidate.commit) {
    fail("legacy receipt normal lane authorization is invalid or stale");
  }
}
...
export function validateCriticRequest(request) {
  ...
  validateLegacyDispatchAuthorization(request.normal_lane_authorization, request.candidate_commit);
...
function loadAcceptedPriorReceipt(controlRoot, priorReceipt, reviewBase) {
  ...
  validateLegacyReceiptAuthorization(record.value);
...
function normalizeCandidateTreePath(value) {
  if (typeof value !== "string" || value.length === 0 || value.length > 240
    || value.includes("\\") || value.startsWith("/") || value.startsWith("./") || value.endsWith("/")) {
    fail("candidate tree contains an invalid path");
  }
  const parts = value.split("/");
  if (parts.some((part) => part === "" || part === "." || part === "..")) fail("candidate tree contains an invalid path");
  return value;
}

/** Candidate blob OIDs are observed from the fixed candidate tree, never guessed from bytes. */
function candidateFileInventory(reviewRoot, candidateCommit) {
  const records = runGit(reviewRoot, ["ls-tree", "-r", "-z", candidateCommit], { encoding: "buffer" }).stdout
    .toString("utf8").split("\0").filter(Boolean);
  return records.map((record) => {
    const match = /^(100644|100755) blob ([0-9a-f]{40,64})\t(.+)$/.exec(record);
    if (!match) fail("candidate tree contains a non-regular or malformed file entry");
    const path = normalizeCandidateTreePath(match[3]);
    const absolute = resolve(reviewRoot, path);
    let readable = false;
    try {
      const info = lstatSync(absolute);
      if (isInside(reviewRoot, absolute) && info.isFile() && !info.isSymbolicLink()) {
        const file = openSync(absolute, "r");
        closeSync(file);
        readable = true;
      }
    } catch {
      // The pure preflight turns unreadability into a deterministic hard failure
      // only when this candidate file is required in the Critic packet.
    }
    return { path, blobOid: match[2], readable };
  });
}

function candidateChangedPaths(reviewRoot, request) {
  return runGit(reviewRoot, ["diff", "--name-only", "-z", request.review_base, request.candidate_commit], { encoding: "buffer" }).stdout
    .toString("utf8").split("\0").filter(Boolean).map(normalizeCandidateTreePath);
}

function criticPacketGovernanceInput(reviewRoot, request) {
  const loadedManifest = loadManifest(reviewRoot);
  if (loadedManifest.status === "invalid") fail("candidate pipeline manifest is unreadable or invalid for Critic governance preflight");
  return {
    schema: "pipeline.critic-packet-governance-input.v1",
    manifest: loadedManifest.status === "absent" ? null : loadedManifest.manifest,
    candidateFiles: candidateFileInventory(reviewRoot, request.candidate_commit),
    changedPaths: candidateChangedPaths(reviewRoot, request),
  };
}

function deriveBoundCriticGovernancePacket(reviewRoot, request) {
  return deriveCriticPacketGovernance(criticPacketGovernanceInput(reviewRoot, request));
}
...
function artifactBindings(reviewRoot, pipelineRoot, request, governancePacket) {
  const result = [
    ...
    ...governancePacket.required.map(({ path }) => ({ source: "review", kind: "governance", ...resolveRegularFile(reviewRoot, path) })),
...
export function prepareNativeCritic(options, deps = {}) {
  ...
  const references = artifactBindings(reviewRoot, pipelineRoot, request, governance);
  ...
  const prepared = {
    ...
    hostContract: {
      ...
      fixedInstruction: [
        "Read only the role, prompt, enumerated commit-set/diff, candidate, spec, guardrails, governance, and evidence references in this packet.",
        "Read every governance.required path and verify its candidateBlobOid before reaching a review conclusion.",
        ...
        `Set achieved_assurance exactly to ${assuranceForRequest(request)}.`,
    ...
    references,
    verify,
    assurance: assuranceForRequest(request),
...
export function validateHostReturn(prepared, preparedSha256, hostReturn, verdictSchema = loadSchema(VERDICT_SCHEMA_PATH)) {
  ...
  if (prepared.assurance !== assuranceForRequest(prepared.request)) fail("prepared assurance boundary drift");
  if (result.achieved_assurance !== prepared.assurance) fail("critic assurance claim mismatch");
...
function validatePrepared(prepared) {
  ...
  const governanceValidation = validateCriticPacketGovernance(
    criticPacketGovernanceInput(prepared.review.root, prepared.request),
    prepared.governance,
  );
  if (!governanceValidation.ok) fail(`prepared Critic governance preflight drift: ${governanceValidation.code}`);
  ...
  for (const { path } of prepared.governance.required) {
    const bound = prepared.references.filter((reference) => reference.source === "review" && reference.kind === "governance" && reference.path === path);
    if (bound.length !== 1) fail("prepared governance reference is missing or ambiguous");
  }
}
...
export function finalizeNativeCritic(options) {
  ...
  validateReviewCommitSet(prepared.review.root, prepared.review);
  const referenceNow = artifactBindings(prepared.review.root, pipelineRoot, prepared.request, prepared.governance);
...
export function parseCliArgs(argv) {
  ...
  if (!new Set(["prepare", "finalize", "selected"]).has(command)) fail("first argument must be prepare, finalize or selected");
  const values = { command, observers: [], cleanup: false };
  const takesValue = new Set(["--repo", "--pipeline-root", "--control-dir", "--dispatch-state", "--request", "--prepared", "--review-dir", "--return", "--receipt", "--observer", "--input"]);
  ...
  const required = command === "selected"
    ? ["input"]
    : command === "prepare"
