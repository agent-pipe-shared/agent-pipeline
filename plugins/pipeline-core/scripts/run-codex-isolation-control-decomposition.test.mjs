...
import { parseArgs } from "./run-codex-isolation-control-decomposition.mjs";

let passed = 0;
function check(name, fn) { fn(); passed += 1; process.stdout.write(`PASS  ${name}\n`); }
check("accepts only a full lower-case commit", () => assert.deepEqual(parseArgs(["--commit", "a".repeat(40)]), { commit: "a".repeat(40) }));
