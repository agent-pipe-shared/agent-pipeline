...
function parse(argv) { if (argv.length !== 4 || argv[0] !== "--repo" || argv[2] !== "--request-file") fail("GAC-ARGUMENT", "Usage: governance-authority.mjs --repo <checkout> --request-file <file>"); return { repositoryRoot: argv[1], requestFile: argv[3] }; }

export async function main(argv = process.argv.slice(2)) {
  const { repositoryRoot, requestFile } = parse(argv);
  let request; try { request = parseStrictJson(await readFile(requestFile)); } catch { fail("GAC-REQUEST", "Authority request must be strict JSON."); }
