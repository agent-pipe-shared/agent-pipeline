...
import { projectGovernanceExportViewStatus } from "../lib/governance-export-view-status.mjs";
function fail(code, message) { const error = new Error(message); error.code = code; throw error; }
function parse(argv) {
  if (argv.length !== 5) fail("GEC-ARGUMENT", "Usage: governance-export.mjs preview --event-file <json> --policy-file <json|none> | map --projection-file <json> --profile-file <json> | view-status --delivery-result-file <json> --boundary-gate-file <json|none>");
  ...
  if (argv[0] === "view-status" && flags.has("--delivery-result-file") && flags.has("--boundary-gate-file")) return { mode: "view-status", deliveryResultFile: flags.get("--delivery-result-file"), boundaryGateFile: flags.get("--boundary-gate-file") };
...
export async function main(argv = process.argv.slice(2), deps = {}) {
  ...
  if (options.mode === "view-status") return (deps.projectViewStatus ?? projectGovernanceExportViewStatus)({ deliveryResult: await load(options.deliveryResultFile), boundaryGate: options.boundaryGateFile === "none" ? null : await load(options.boundaryGateFile) });
