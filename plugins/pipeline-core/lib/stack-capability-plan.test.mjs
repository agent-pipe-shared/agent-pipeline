...
const threatModel = { candidate, digest: "c".repeat(64) };
const input = { candidate, discovery, policyRevision: "policy-1", threatModel, observations: STACK_CAPABILITIES.map((capability) => ({ capability, present: capability !== "cap.dast" && capability !== "cap.fuzz" })), requirements: [{ capability: "cap.sast", required: true }, { capability: "cap.dast", required: true }] };
...
assert.equal(buildStackCapabilityPlan({ ...input, threatModel: { candidate, digest: "not-a-digest" } }).code, "STACK-PLAN-INVALID");
