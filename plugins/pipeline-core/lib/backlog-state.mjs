...
const LEGACY_EVIDENCE_AMENDMENT_KEYS = new Set(["kind", "commit", "reference", "previousClosureCommit", "resultSha256", "privateLicenseGateSha256", "neutralPublicLicenseGateSha256"]);
...
function validateTransitionShape(event, label, { readDispositionBytes = null, authorizeAmendment = null, authorizeOrdinaryEvidence = null } = {}) {
  ...
  const amendment = !isV2 && event.from === "closed" && event.to === "closed" && event?.evidence?.kind === "evidence-amendment";
  const v2Amendment = isV2 && event?.evidence?.kind === "evidence-amendment";
  const afkRepair = event.id === AFK_REPAIR_ID && event.from === null && event.to === "open" && event?.evidence?.kind === "missing-initial-ledger-repair";
  if (event.from === event.to && !amendment && !v2Amendment) errors.push(`${label}: transition must change status`);
  if (v2Amendment && (!(event.from === event.to) || !BACKLOG_STATUSES.includes(event.from))) errors.push(`${label}: v2 evidence amendment must preserve one canonical status`);
  if (isV2 && !v2Amendment) {
    const allowed = (event.from === null && event.to === "open")
      || (event.from === "open" && event.to === "in_progress")
      || (event.from === "in_progress" && event.to === "closed");
    if (!allowed) errors.push(`${label}: v2 ordinary transition must be null -> open, open -> in_progress, or in_progress -> closed`);
  }
  ...
  if (!isPlainObject(event.evidence)) errors.push(`${label}: evidence must be an object`);
    ...
    const evidenceKeys = amendment ? LEGACY_EVIDENCE_AMENDMENT_KEYS : afkRepair ? AFK_REPAIR_EVIDENCE_KEYS : new Set(["kind", "commit", "legacyStatus", "reference"]);
...
export function validateTransitionLedger(events, items, { commitExists = null, readDispositionBytes = null, authorizeAmendment = null, authorizeOrdinaryEvidence = null } = {}) {
  ...
  for (const [index, event] of events.entries()) {
    ...
    if (event?.evidence?.kind === "missing-initial-ledger-repair" && event.evidence.sourceSha256 !== createHash("sha256").update(asString(item?.metadata?.source)).digest("hex")) mark(index, `${label}: sourceSha256 does not bind the current item source`);
    ...
    if (prior === undefined) {
      ...
      if (prior === "closed") {
        ...
        } else if (!(event.from === "closed" && event.to === "closed" && event?.evidence?.kind === "evidence-amendment")) {
          mark(index, `${label}: closed must never transition to another status`);
        } else if (event.evidence.previousClosureCommit !== closureCommitById.get(event.id)) {
          mark(index, `${label}: previousClosureCommit does not bind the prior closure`);
        }
      } else if (v2Amendment) {
        if (event.to !== prior) mark(index, `${label}: evidence amendment must not mutate status`);
      } else if (FORWARD_TRANSITIONS[prior] !== event.to) {
        mark(index, `${label}: ${prior} may only move to ${FORWARD_TRANSITIONS[prior]}`);
      }
    }
    if (BACKLOG_STATUSES.includes(event.to)) stateById.set(event.id, event.to);
    const projectedCommit = projectedClosureCommit(event);
    if (event.to === "closed" && OID.test(asString(projectedCommit))) closureCommitById.set(event.id, projectedCommit);
    const externalProjectClosure = item?.metadata?.closure_repository?.startsWith("project:") && event.to === "closed";
    if (!externalProjectClosure && OID.test(asString(event?.evidence?.commit))) {
      localEvidenceCommits.push({ index, commit: event.evidence.commit });
    }
  ...
  for (const [id, item] of itemById) {
    ...
    if (item.metadata.status === "closed" && current === "closed") {
      ...
      if (item.metadata.status === "closed" && current === "closed" && isV2EvidenceAmendment(final)
        && item.metadata.closure_evidence !== final.evidence.reference) {
