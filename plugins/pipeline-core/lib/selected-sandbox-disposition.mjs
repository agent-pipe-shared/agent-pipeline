...
function validFingerprint(value) {
  return exact(value, FINGERPRINT) && SHA256.test(value.runnerSha256) && SHA256.test(value.hostBootSha256)
    && string(value.platformClass) && string(value.architectureClass) && SHA256.test(value.sandboxSha256)
    && SHA256.test(value.profileSha256) && SHA256.test(value.policySha256) && string(value.duty) && string(value.contractVersion);
}
function validAttempt(value) {
  return exact(value, ["attemptId", "index", "startedMonotonicMs"]) && string(value.attemptId)
...
function validReceipt(value) {
  return exact(value, ["childIdSha256", "attemptId", "nonceSha256", "duty", "transport", "subjectSha256", "assurance", "resultSha256", "terminal"])
    && SHA256.test(value.childIdSha256) && string(value.attemptId) && SHA256.test(value.nonceSha256)
    && string(value.duty) && string(value.transport) && SHA256.test(value.subjectSha256)
...
function validAuthority(value) {
  return exact(value, ["kind", "decisionId", "receiptPath", "receiptSha256"]) && value.kind === "po-force-reprobe"
    && string(value.decisionId) && string(value.receiptPath) && !value.receiptPath.startsWith("/") && SHA256.test(value.receiptSha256);
}
...
function validationCode(record, verifyDigest = true) {
  ...
  if (!string(record.dispositionId) || !string(record.duty) || !string(record.transport) || !SHA256.test(record.recordSha256)) return "SHAPE:scalar";
  ...
  if (record.state === "available-attested") {
    if (!record.childReceipt || record.failure !== null || record.expiry.kind !== "none" || record.assurance.observed !== ASSURANCE_VALUE
      || record.assurance.evidenceSha256 === null || record.childReceipt.attemptId !== record.attempt.attemptId
      || record.childReceipt.nonceSha256 !== record.challenge.nonceSha256 || record.childReceipt.duty !== record.duty
      || record.childReceipt.transport !== record.transport || record.childReceipt.subjectSha256 !== record.fingerprint.runnerSha256
      || record.childReceipt.resultSha256 !== record.fingerprint.hostBootSha256) return "AUTHORITY:attestation";
  } else if (record.childReceipt !== null || (record.state !== "probing" && hasProbe)) return "BOUND:state-material";
  if (record.state === "unprobed" && (record.failure !== null || record.previousSha256 !== null || record.expiry.kind !== "none")) return "BOUND:unprobed";
  if (record.state === "probing" && (record.failure !== null || record.expiry.kind !== "none")) return "BOUND:probing";
  if (record.state === "transient-unavailable" && (!record.failure || record.failure.class !== "transient-unavailable" || record.expiry.kind !== "transient-ttl" || record.expiry.eligibleAfterMonotonicMs === null)) return "BOUND:transient";
  if (record.state === "terminal-unavailable" && (!record.failure || record.failure.class !== "terminal-unavailable" || record.expiry.kind !== "session" || record.expiry.eligibleAfterMonotonicMs !== null)) return "BOUND:terminal";
  if (record.state === "invalidated" && (record.failure !== null || record.expiry.kind !== "fingerprint-drift" || record.expiry.eligibleAfterMonotonicMs !== null)) return "BOUND:invalidated";
...
function toProbing(record, event, forceReprobe = record.forceReprobe) {
  ...
  if (record.attempt && event.attempt.index <= record.attempt.index) return null;
...
export function reduceSelectedSandboxDisposition(record, event) {
  ...
  if (event.kind === "fingerprint-drift") {
    ...
    const disposition = advance(record, { fingerprint: clone(event.fingerprint), state: "invalidated", failure: null, attempt: null, challenge: null, childReceipt: null,
      expiry: { kind: "fingerprint-drift", eligibleAfterMonotonicMs: null }, assurance: { requested: "selected-sandbox", observed: "not-observed", evidenceSha256: null } });
  ...
  if (record.state === "probing" && event.kind === "probe-success") {
    ...
    if (receipt.childIdSha256 !== event.selectedChildIdSha256 || receipt.attemptId !== record.attempt.attemptId || receipt.nonceSha256 !== record.challenge.nonceSha256
      || receipt.duty !== record.duty || receipt.transport !== record.transport || receipt.subjectSha256 !== record.fingerprint.runnerSha256
      || receipt.resultSha256 !== record.fingerprint.hostBootSha256) return reject("AUTHORITY:receipt-binding");
  ...
  if (record.state === "probing" && event.kind === "probe-failure") {
    ...
    const disposition = advance(record, { state: event.failure, failure: { class: event.failure, observationReceiptSha256: event.observationReceiptSha256 }, attempt: null, challenge: null,
