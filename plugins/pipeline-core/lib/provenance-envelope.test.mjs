...
const expected = { candidate, subject, materials: envelope.materials, builderDigest: envelope.builder.digest };
...
for (const boundary of ["produce", "promote", "readback"]) assert.deepEqual(evaluateProvenanceAdmission({ boundary, envelope, expected }), { allowed: true, code: "PROVENANCE-ADMISSION-ALLOWED" });
assert.equal(evaluateProvenanceAdmission({ boundary: "promote", envelope, expected: { ...expected, subject: { ...subject, sha256: hash("9") } } }).code, "PROVENANCE-SUBJECT-MISMATCH");
assert.equal(evaluateProvenanceAdmission({ boundary: "readback", envelope: { ...envelope, attestation: { ...envelope.attestation, status: "revoked" } }, expected }).allowed, false);
assert.equal(evaluateProvenanceAdmission({ boundary: "produce", envelope: { ...envelope, materials: [{ ...envelope.materials[0], sha256: hash("8") }] }, expected }).code, "PROVENANCE-MATERIAL-MISMATCH");
