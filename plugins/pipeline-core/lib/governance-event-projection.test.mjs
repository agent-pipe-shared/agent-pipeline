...
function actionEvent() { const candidate = { commit: "a".repeat(40), tree: "b".repeat(40) }; const payload = buildGovernanceActionEvent({ kind: "verification", status: "completed", reasonCode: "VERIFICATION_PASSED", requestId: "verify-request-1", featureId: unavailable, sessionId: unavailable, candidate }); return event({ payloadSchema: "pipeline.governance-action-event.v1", eventId: payload.eventId, idempotencyKey: payload.correlation.actionId, eventType: "lifecycle.action.verification", correlation: { featureId: unavailable, packageId: unavailable, requestId: payload.correlation.requestId, sessionId: unavailable, dispatchId: unavailable, traceId: payload.correlation.actionId }, candidate, payload }); }
...
test("projects action envelopes through the unchanged envelope-only allowlist", () => {
  ...
  assert.doesNotMatch(JSON.stringify(result.projection), /verify-request-1|actionId|reasonCode|VERIFICATION_PASSED|payload/);
});
