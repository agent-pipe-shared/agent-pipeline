...
test("fails closed for repository/candidate drift, expiry, and consuming disposition", () => {
  ...
  const consumed = decision({ decisionId: "consume-1", event: "consumed", outcome: "consumed", links: { requestDecisionId: null, consumesDecisionId: "decision-1", revokesDecisionId: null, supersedesDecisionId: null, correctsDecisionId: null } });
...
test("rejects open payloads and invalid lifecycle link cardinality", () => {
  ...
  assert.throws(() => validateHumanGovernanceDecision(decision({ links: { requestDecisionId: null, consumesDecisionId: null, revokesDecisionId: null, supersedesDecisionId: null, correctsDecisionId: null } })), (error) => error instanceof HumanGovernanceLedgerError && error.code === "HGL-LIFECYCLE");
});
