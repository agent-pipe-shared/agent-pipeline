...
/** E-AC-08 outbox truncation: no stored entry may vanish or have its projection replaced; attempts/status/cursor legitimately advance and are deliberately not compared. */
function truncates(current, next) { return current.entries.some((entry, index) => !next.entries[index] || JSON.stringify(next.entries[index].projection) !== JSON.stringify(entry.projection)); }
/** E-AC-08 cursor rollback: a genuine decrease between two writes is a defect; staying the same or advancing by any amount remains legal. */
function rollsBackCursor(current, next) { return next.cursor < current.cursor; }
