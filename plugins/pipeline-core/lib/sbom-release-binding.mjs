...
export function bindSbomRelease(history, { release, manifest, validation, manifestDigest, components }) {
  if (!Array.isArray(history) || !history.every(validRelease) || !string(release) || !HEX.test(manifestDigest ?? "") || !Array.isArray(components) || !components.every(string)) return { ok: false, code: "SBOM-RELEASE-INPUT-INVALID" };
  ...
  const prior = history.at(-1); const previous = new Set(prior?.components ?? []); const current = new Set(components);
