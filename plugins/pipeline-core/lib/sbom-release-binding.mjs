...
import { canonicalJson, validateSbomManifest, validateSbomRecord } from "./sbom-manifest.mjs";
...
export function bindSbomRelease(history, { release, manifest, payloads, observedCandidate, validation }) {
  ...
  const record = validateSbomRecord(manifest, payloads, observedCandidate);
  if (manifest?.lifecycle?.state !== "complete" || manifest?.freshness?.status !== "fresh" || manifest?.completeness?.status !== "complete" || !record.valid || validation?.valid !== true || validation?.digest !== record.digest || !HEX.test(validation?.digest ?? "")) return { ok: false, code: "SBOM-RELEASE-PRECONDITION" };
  ...
  const entry = Object.freeze({ release, sbomDigest: record.digest, manifestDigest, components: [...current].sort(), delta: Object.freeze({ added: [...current].filter((id) => !previous.has(id)).sort(), removed: [...previous].filter((id) => !current.has(id)).sort() }) });
