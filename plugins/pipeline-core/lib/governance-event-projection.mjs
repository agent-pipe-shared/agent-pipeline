...
export function createGovernanceDeliveryReceipt(input) {
  if (!exact(input, ["destinationProfile", "policyRevision", "projectionDigest", "batchId", "eventCount", "attempt", "acknowledgementClass", "terminalDisposition", "cursor", "lag"]) || !ID.test(input.destinationProfile) || !SHA.test(input.policyRevision) || !SHA.test(input.projectionDigest) || !ID.test(input.batchId) || !Number.isSafeInteger(input.eventCount) || input.eventCount < 0 || !Number.isSafeInteger(input.attempt) || input.attempt < 1 || !new Set(["none", "partial", "accepted"]).has(input.acknowledgementClass) || !new Set(["pending", "delivered", "retryable-failure", "quarantined"]).has(input.terminalDisposition) || !Number.isSafeInteger(input.cursor) || input.cursor < 0 || !Number.isSafeInteger(input.lag) || input.lag < 0) fail("GEP-RECEIPT");
