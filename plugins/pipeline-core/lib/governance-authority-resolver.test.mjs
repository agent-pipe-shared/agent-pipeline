...
import { requireExternallyAttestedGovernanceAuthority, requireGovernanceAuthority } from "./governance-authority-resolver.mjs";
...
test("external resolver refuses locally-attributed grants until their detached proof verifies", () => {
  ...
  assert.equal(requireExternallyAttestedGovernanceAuthority(request).identityAssurance, "externally-attested");
  assert.deepEqual(requireExternallyAttestedGovernanceAuthority({ ...request, proof: { ...proof, signatureBase64: "AA==" } }), { granted: false, reason: "external-proof-unverified", proofCode: "HGL-EXTERNAL-PROOF-MISMATCH" });
});
