...
test("accepts only the second bounded fallback attempt", () => {
  const launch = createHostAdvisorLaunch("fallback", { randomBytes:()=>Buffer.alloc(32, 9) });
  const status = createHostAdvisorStatus({
    candidate:{commit:c,tree:c},
    launch,
    questionSha256:h,
    workspaceBeforeSha256:h,
    workspaceAfterSha256:h,
    answerSha256:h,
    outcome:"answered",
    agentName:"consult-advisor-fast",
    attemptCount:2,
  });
  assert.equal(status.attempt.agentName, "consult-advisor-fast");
  assert.equal(status.attempt.count, 2);
  assert.doesNotThrow(()=>validateHostAdvisorStatus(status,{commit:c,tree:c},launch,h));
  assert.throws(()=>createHostAdvisorStatus({
    candidate:{commit:c,tree:c},
    launch,
    questionSha256:h,
    workspaceBeforeSha256:h,
