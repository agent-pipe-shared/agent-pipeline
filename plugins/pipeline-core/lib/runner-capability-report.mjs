...
const isPath = (value) => isString(value) && Buffer.byteLength(value, "utf8") <= 512;
...
function validateEvidence(value) {
  return exact(value, ["kind", "path", "fileSha256", "recordSha256"])
    && isId(value.kind) && (value.path === null || isPath(value.path))
...
function validateCell(value) {
  if (!exact(value, ["capabilityId", "mode", "status", "evidence"]) || !isId(value.capabilityId)
    || !MODES.has(value.mode) || !CELL_STATUSES.has(value.status) || !Array.isArray(value.evidence)
    || value.evidence.length > 64 || !value.evidence.every(validateEvidence)
    || !sortedUnique(value.evidence, (item) => `${item.kind}\u0000${item.path ?? ""}\u0000${item.fileSha256}\u0000${item.recordSha256 ?? ""}`)) return false;
  ...
  return ["certified", "observed"].includes(value.status) && value.evidence.length > 0;
}
...
function validationCode(record, verifyDigest = true) {
  ...
  if (!isId(record.reportId) || !SHA256.test(record.recordSha256)) return "SHAPE:scalar";
  if (!exact(record.identity, ["adapterId", "adapterVersion", "implementationSha256", "requestedRunner", "requestedModel", "observedRunner", "observedModel"])
    || !isId(record.identity.adapterId) || !isId(record.identity.adapterVersion) || !SHA256.test(record.identity.implementationSha256)
    || !isId(record.identity.requestedRunner) || !isId(record.identity.requestedModel)
    || !(record.identity.observedRunner === "not-observed" || isId(record.identity.observedRunner))
    || !(record.identity.observedModel === "not-observed" || isId(record.identity.observedModel))) return "SHAPE:identity";
  ...
  if (!exact(record.environment, ["platformClass", "architectureClass", "hostClass", "fingerprintSha256"])
    || !isId(record.environment.platformClass) || !isId(record.environment.architectureClass)
    || !isId(record.environment.hostClass) || !SHA256.test(record.environment.fingerprintSha256)) return "SHAPE:environment";
  if (!Array.isArray(record.capacity) || record.capacity.length === 0 || record.capacity.length > 256 || !record.capacity.every(validateCapacity)
    || !sortedUnique(record.capacity, (item) => `${item.unit}\u0000${item.source}`)) return "BOUND:capacity";
  if (!Array.isArray(record.cells) || record.cells.length === 0 || record.cells.length > 256 || !record.cells.every(validateCell)
    || !sortedUnique(record.cells, (item) => item.capabilityId)) return "BOUND:cells";
  const cellsById = new Map(record.cells.map((cell) => [cell.capabilityId, cell]));
  const synthetic = cellsById.get("synthetic-contract");
  const claude = cellsById.get("claude-native-review");
  const codex = cellsById.get("codex-native-readonly");
  const unsupported = cellsById.get("unsupported-live-worker");
  if (synthetic?.mode !== "synthetic" || synthetic.status !== "certified"
    || !claude || !["native", "unavailable"].includes(claude.mode)
    || !codex || !["native", "unavailable"].includes(codex.mode)
    || unsupported?.mode !== "unsupported" || unsupported.status !== "unsupported") return "BOUND:baseline-matrix";
  ...
  for (const section of Object.values(record.assurance)) {
    if (!exact(section, ["status", "evidenceSha256"]) || !ASSURANCE_STATUSES.has(section.status)
      ...
      || (section.status === "not-observed" && section.evidenceSha256 !== null)
      || (section.status === "observed" && !SHA256.test(section.evidenceSha256))) return "SHAPE:assurance-value";
  }
  if (!exact(record.bindings, ["suiteVersion", "candidate", "authorityDigests", "rawEvidenceSha256"])
    || !isId(record.bindings.suiteVersion) || !exact(record.bindings.candidate, ["commit", "tree"])
    || !GIT_SHA.test(record.bindings.candidate.commit) || !GIT_SHA.test(record.bindings.candidate.tree)
    || !Array.isArray(record.bindings.authorityDigests) || record.bindings.authorityDigests.length === 0 || record.bindings.authorityDigests.length > 256
    || !record.bindings.authorityDigests.every((item) => exact(item, ["kind", "sha256"]) && isId(item.kind) && SHA256.test(item.sha256))
    || !sortedUnique(record.bindings.authorityDigests, (item) => `${item.kind}\u0000${item.sha256}`)
    || !Array.isArray(record.bindings.rawEvidenceSha256) || record.bindings.rawEvidenceSha256.length === 0 || record.bindings.rawEvidenceSha256.length > 64
    || !record.bindings.rawEvidenceSha256.every((item) => SHA256.test(item))
