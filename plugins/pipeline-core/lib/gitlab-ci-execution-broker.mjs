...
export function reconcileGitLabCiBroker(record, observation) { if (!validateGitLabCiBroker(record).ok || record.state !== "succeeded-unverified" || !validObservation(observation) || !record.job || observation.sequence !== record.observations.length + 1 || observation.providerState !== "success" || observation.jobIdSha256 !== record.job.jobIdSha256 || observation.candidateCommit !== record.candidate.commit || observation.targetSha256 !== targetDigest(record.target) || observation.observedAt < record.observations.at(-1).observedAt || observation.observationSha256 !== obsDigest({ ...observation, observationSha256: undefined })) return { ok: false, code: "AUTHORITY:broker-reconcile", broker: null }; const next = clone(record); next.observations.push(clone(observation)); next.previousSha256 = record.recordSha256; next.state = "reconciled"; return { ok: true, code: null, broker: seal(next) }; }
