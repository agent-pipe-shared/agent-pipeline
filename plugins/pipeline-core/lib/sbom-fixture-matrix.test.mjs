...
const base = { schema: NODE_GRAPH_SCHEMA, components: [{ id: "pkg:npm/a@1", scope: "app", provenance: "apps/app/package-lock.json", name: "a", version: "1", dependencies: [] }] };
...
test("multi ecosystem fixture semantics", () => assert.equal(generateNodeSbom({ ...base, components: [...base.components, { id: "pkg:other/b@1", scope: "other", provenance: "other.lock", name: "b", version: "1", dependencies: [] }] }).valid, true));
test("monorepo preserves separate scopes and provenance", () => { const result = JSON.stringify(generateNodeSbom({ ...base, components: [...base.components, { id: "pkg:npm/b@1", scope: "packages/b", provenance: "packages/b/package-lock.json", name: "b", version: "1", dependencies: [] }] }).cyclonedx); assert.match(result, /packages\/b/); assert.match(result, /packages\/b\/package-lock\.json/); });
...
test("lossy conversion preserves relationships and provenance", () => { const graph = { ...base, components: [{ ...base.components[0], dependencies: ["pkg:npm/b@1"] }, { id: "pkg:npm/b@1", scope: "lib", provenance: "packages/lib/package-lock.json", name: "b", version: "1", dependencies: [] }] }; const result = generateNodeSbom(graph); assert.match(result.spdx.relationships[0].relatedSpdxElement, /^SPDXRef-[A-Za-z0-9.-]+$/u); assert.equal(result.spdx.packages.find((pkg) => pkg.externalRefs[0].referenceLocator === "pkg:npm/b@1").SPDXID, result.spdx.relationships[0].relatedSpdxElement); assert.match(JSON.stringify(result.spdx), /packages\/lib\/package-lock\.json/); });
