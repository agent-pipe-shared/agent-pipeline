...
test("lossy conversion preserves relationships", () => { const graph = { ...base, components: [{ ...base.components[0], dependencies: ["pkg:npm/b@1"] }, { id: "pkg:npm/b@1", scope: "lib", name: "b", version: "1", dependencies: [] }] }; const result = generateNodeSbom(graph); assert.match(result.spdx.relationships[0].relatedSpdxElement, /^SPDXRef-[A-Za-z0-9.-]+$/u); assert.equal(result.spdx.packages.find((pkg) => pkg.externalRefs[0].referenceLocator === "pkg:npm/b@1").SPDXID, result.spdx.relationships[0].relatedSpdxElement); });
