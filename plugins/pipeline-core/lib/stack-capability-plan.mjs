...
const digest = (candidate, discoveryDigest, policyRevision, entries) => createHash("sha256").update(JSON.stringify({ candidate, discoveryDigest, policyRevision, entries })).digest("hex");
...
export function buildStackCapabilityPlan(input) {
  ...
  const planDigest = digest(input.candidate, input.discovery.digest, input.policyRevision, entries);
  return { ok: true, schema: "pipeline.stack-capability-plan.v1", candidate: structuredClone(input.candidate), discoveryDigest: input.discovery.digest, policyRevision: input.policyRevision, entries, digest: planDigest };
}
...
export function validateStackCapabilityPlan(plan) {
  if (!own(plan, ["ok", "schema", "candidate", "discoveryDigest", "policyRevision", "entries", "digest"]) || plan.ok !== true || plan.schema !== "pipeline.stack-capability-plan.v1" || !own(plan.candidate, ["commit", "tree"]) || !oid(plan.candidate.commit) || !oid(plan.candidate.tree) || plan.candidate.commit === plan.candidate.tree || !/^[a-f0-9]{64}$/u.test(plan.discoveryDigest) || typeof plan.policyRevision !== "string" || plan.policyRevision === "" || !entries(plan.entries) || !/^[a-f0-9]{64}$/u.test(plan.digest) || plan.digest !== digest(plan.candidate, plan.discoveryDigest, plan.policyRevision, plan.entries)) return { valid: false, code: "STACK-PLAN-INVALID" };
  return { valid: true };
}
