  ...
  try {
    ...
    const state = (argv) => {
      ...
      const code = pipelineStateRun(argv, {
        dir: path,
        now: () => "2026-08-01T12:00:00.000Z",
        writeError: (v) => stderr.push(String(v)),
...
test("bootstrap-binding-required routes a hand-authored staging PRD through its policy-selected acknowledgement and then binds", () => {
  ...
  try {
    ...
    // A generated PRD is now subject to the same policy-selected acknowledgement as
    // an authored one. Otherwise a runner can bind the generator draft, change it,
    // and later reach the legacy `approve-plan --by` path under signature mode.
    ...
    assert.equal(exemptObservation.exempt, false, "generated bytes are not exempt from the shared human gate");
    ...
    assert.notEqual(exemptInspect.nextAction.argv?.[1], "bootstrap-bind-plan", "a marker-less draft must not bind before acknowledgement");
    assert.throws(() => planOnboardingBootstrapBind({ rootDir: path, repositoryCapability: "local", spawn: fakeGit }),
      (error) => error?.code === "KICKOFF-PROMOTION-PRD-ACKNOWLEDGEMENT-MARKER-MISSING");

    // An authored change keeps the same acknowledgement requirement and must get a
    // new intent if it is made after the earlier signature plan.
    ...
    const chatDeps = {
      ...fakeDeps,
      spawn: fakeGit,
      readHumanApprovalMode: () => ({ mode: "chat", source: "test", key: "gates.human_approval", scope: "global" }),
    };
    let chatAction = null;
    for (const runner of ["claude", "codex", "antigravity"]) {
    ...
    const signatureDeps = {
      ...
      readHumanApprovalMode: () => ({ mode: "signature", source: "test", key: "gates.human_approval", scope: "global" }),
    ...
    // Signature posture uses the existing shared proof module; inspection
    // offers its external sign action to every runner rather than a fakeable
    // in-session acknowledgement command.
    ...
    for (const runner of ["claude", "codex", "antigravity"]) {
      ...
      assert.equal(awaitingHuman.nextAction.executionBoundary, "attended-external-tool", runner);
      assert.equal(awaitingHuman.nextAction.action.argv[1], "sign-intent", runner);
      assert.equal(awaitingHuman.nextAction.requiresConfirmation, true, runner);
      assert.equal(typeof awaitingHuman.nextAction.action.command, "string", runner);
    ...
    for (const runner of ["claude", "codex", "antigravity"]) {
      ...
      assert.equal(unrestricted.nextAction.action.argv[1], "sign-intent", runner);
    ...
    // A stray invalid proof cannot manufacture acknowledgement; every runner
    // continues to expose only the external sign action.
    for (const runner of ["claude", "codex", "antigravity"]) {
      ...
      assert.equal(invalidProof.nextAction.action.argv[1], "sign-intent", runner);
    }
    assert.throws(() => applyOnboardingBootstrapAcknowledgement({
      rootDir: path, repositoryCapability: "local", spawn: fakeGit, activate: true,
    ...
    assert.equal(acknowledgementApplied.acknowledgementMode, "signature");
...
test("chat acknowledgement consumes the one in-chat PO decision without a second terminal confirmation", () => {
  ...
  try {
    ...
    const generated = planOnboardingIntakeGenerate({ rootDir: path, repositoryCapability: "local", spawn: fakeGit });
    applyOnboardingIntakeGenerate({
      rootDir: path, repositoryCapability: "local", expectedPlanSha256: generated.planSha256, activate: true, deps: { spawn: fakeGit },
    });
    const observation = observeBootstrapBindAcknowledgement({ rootDir: path, repositoryCapability: "local", spawn: fakeGit });
    ...
    const common = {
      ...
      spawn: fakeGit,
      expectedPlanSha256: plan.intentSha256,
      attendedChat: true,
      activate: true,
    };
    assert.throws(() => applyOnboardingBootstrapAcknowledgement({
...
// NVA-B-UNLINK-1. `applyProjectOnboardingManifestRepair` used to assign
// `published` only AFTER `fs.unlinkSync(temp)` returned, on the same crammed
// line as the `fs.linkSync` that made the target live
// (`fs.linkSync(temp, target); fs.unlinkSync(temp); temp = null; published =
// target;`). A throwing `unlinkSync(temp)` right there left `published` at
// its initial `null`, so the catch had nothing to roll back while the target
// was in fact live at its path -- the function returned `status:
// "rolled-back"` for a manifest that was genuinely published. This asserts
// the item's own Acceptance invariant in typed form: whatever status is
// reported, it must match what is actually on disk.
test("NVA-B-UNLINK-1 V3 manifest repair does not claim rolled-back while a throwing temporary unlink leaves the target published", () => {
  const path = readyManifestFixture();
  try {
    const plan = planProjectOnboardingManifestRepair({ rootDir: path, deps: fakeDeps });
    assert.equal(plan.status, "ready", JSON.stringify(plan));
    const throwing = {
      ...fakeDeps,
      unlinkSync(candidate) {
        if (typeof candidate === "string" && candidate.includes(".pipeline-manifest-repair-") && candidate.endsWith(".tmp")) {
          throw new Error("synthetic temporary-unlink failure");
        }
        return unlinkSync(candidate);
      },
    };
    const result = applyProjectOnboardingManifestRepair({
      runner: "codex", rootDir: path, planSha256: plan.planSha256, activate: true, deps: throwing,
    });
    const target = join(path, ".claude", "pipeline.yaml");
    const targetIsOurs = existsSync(target) && sha256(readFileSync(target, "utf8")) === plan.generated.sha256;
    // The item's own Acceptance point 1, typed: a "rolled-back" claim must
    // never coexist with the target still holding this transaction's own
    // published bytes.
    if (result.status === "rolled-back") {
      assert.equal(targetIsOurs, false,
        `status claims rolled-back but the target is still published with this transaction's own content: ${JSON.stringify(result)}`);
    }
    // The concrete shape this fix produces: publication is real and stays
    // real (the transaction cannot safely delete a target while its
    // temporary is still hard-linked to it -- nlink stays 2, so the existing
    // content-bound ownsPublishedOutput() correctly refuses), and the status
    // says so instead of claiming a rollback that did not happen.
    assert.equal(targetIsOurs, true, `the target should still be published with the generated bytes: ${JSON.stringify(result)}`);
    assert.equal(result.status, "rollback-failed", JSON.stringify(result));
  } finally { dispose(path); }
});
