...
import { chmodSync, existsSync, linkSync, mkdirSync, readFileSync, symlinkSync, writeFileSync, rmSync } from "node:fs";
...
// This suite used the operator's home directory for its ordinary fixtures,
// while its Windows branch already used the repository-local test scratch.
// Keep every fixture under the same sanctioned, writable repository root so a
// restricted Codex sandbox can exercise the test without needing a mutable
// home directory.
const fixture = (fn) => { const root = mkdtempTestScratch("local-supervisor-"); try { fn(root); } finally { rmSync(root, { recursive: true, force: true }); } };
