...
- **Guard config (only if needed):** `.claude/guard-config.json`
  ...
  with none.
  Agent-authored commits default to `commitTrailerPolicy: blocking` even when
  this project has no guard-config yet; this requires exactly one grounded
  `Dispatch:` binding plus `AI-Assisted: true` in the final Git trailer block.
  `warn` is suitable only for a dated migration, and `off` is an explicit
  opt-out from these two provenance conventions.
