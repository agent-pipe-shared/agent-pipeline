...
- **Independent Critic is mandatory for every Goldfish delivery.** Your opening v3 record MUST contain one conservative `criticRequired` decision: schema `pipeline.critic-required-decision.v1`, trigger `{ schema: "pipeline.critic-trigger-input.v1", rigorLevel: 2, riskClass: "low", riskFlag: false, diff: { mechanical: false, architecture: false, guardrails: false, security: false } }`, and `appliedRow: "T3"`. Do not create `criticSkip`, decide that a Critic may be skipped, replace the pending decision, or describe the delivery as complete. Only the Elephant may dispatch the independent Critic; only its task/candidate/path/digest-bound `criticEvidence` receipt completes this gate. Your final report must say `independent review: pending` unless that independently persisted receipt has already been supplied to you.
...
- **Create the dispatch record FIRST, before any other work.** Write the v3 record as your opening act with `taskId`, `model`, `rulesetSha`, `dispatcher`, `outcome: "in-progress"`, an empty `log`, and the mandatory conservative `criticRequired` decision above. Append to it as you go. This is not bookkeeping order, it is the only thing that makes a truncated run diagnosable: dispatches have ended mid-sentence with the words "now let's write the dispatch record", leaving no record at all and nothing to read afterwards. A record created at the end exists exactly when it is no longer needed. Cost: one write. Immediately after this opening write, read the file back once to confirm it exists on disk before proceeding with the rest of the task; a failed readback is a stop condition (missing access/tool/permission) — never proceed as if the write succeeded. A report has previously stated a dispatch record was created when it was not (`backlog/items/2026-08-08-a-dispatch-reported-creating-a-record-it-never-created.md`).
