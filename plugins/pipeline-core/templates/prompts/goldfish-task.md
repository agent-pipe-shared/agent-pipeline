...
- Budget direction: `pipeline.maxturns-budget-direction-decided` — the authenticated Claude hook enforces the base cap; another runner needs its own authenticated live-call adapter before the same enforcement may be claimed.
...
- **The closing handover is STRUCTURED, not prose (`pipeline.dispatch-closing-allowance.v1`).** "I ran out of budget" is what happens today and is worthless to the dispatcher. Capture the handover adhering to schema `pipeline.dispatch-closing-allowance.v1` (and record it in `closingAllowance` if writing the record), and make these four statements explicit and separately findable in the final report — each of them, including when the answer is "none":
  1. **Committed:** the commit SHA(s) and what each contains — or, in those words, "nothing committed" (`committed: string[]`).
  2. **Verified green:** which DoD checks/suites actually ran green, each with its exact command, exit code and machine-written artifact path (`verifiedGreen: object[]`). A check you never ran is not green; a check that ran red is not green.
  3. **Remains undone:** which DoD checks and which parts of the goal are untouched or half-done, named one by one — plus every file left modified and uncommitted (`remainsUndone: string[]`).
  4. **What the next briefing would have to say differently** to finish the rest: the split, the missing context file, the assumption that turned out wrong, the cap that was too small (`nextBriefingAdjustments: string[]`). This is what stops a re-dispatch from failing in the same shape, and only you can write it, because you are the run that hit the wall.
...
- **Dispatch record (standard evidence):** write `evidence/dispatch-record-{{TASK_ID}}.json` — the task-ID-suffixed name is the STANDARD one, not a variant, and it is the only name the authorship checker (`dispatch-authorship-verify`) resolves a `Dispatch:` trailer to; never the fixed name `dispatch-record.json`, which collides when two dispatches land in the same evidence directory. Keep the standard record directly under `evidence/`; the authorship and coverage checkers resolve that exact location. Use `schema: "pipeline.dispatch-record.v3"`. Fields: `taskId`, `agentType`, `model`, `effort`, `rulesetSha`, `dispatcher`, `outcome`, plus `commits` (array, appended one SHA per commit-then-checkpoint step — see Report durability below), `log` (append-only running entries), `report`, and exactly one Critic disposition described below. **`effort` is not optional once `agentType` is declared:** the authorship checker's model-vs-definition dimension (see `agentType` below) compares `model` AND `effort` together against the dispatched agent's own definition, and treats an absent `effort` as a plain string mismatch, not as unknown — a record that names `agentType` but omits `effort` is downgraded to FAIL, classification `model-mismatch`, even when the commit it vouches for is genuinely authored (confirmed empirically: `evidence/dispatch-record-NVA-B-EFFORTFIELD.json`). **`report` is an OBJECT with two REQUIRED fields**, not a bare string: `report.text` (your final report prose) and `report.changedFiles` (an array of the repo-relative paths this dispatch changed — each entry either a bare path, `"<path> - why it changed"`, or `{ "path": "<path>" }`). Permissible orchestrator additions (EL-01 / EL-16 registry appends, ADRs, or handover notes added by the orchestrator at commit time) may be disclosed in `report.orchestratorAddedFiles` (or top-level `orchestratorAddedFiles`) so the authorship verifier admits them without misattributing them to the dispatch. A `report` written as bare prose carries no machine-readable path list, so the authorship check can only answer UNVERIFIABLE about it. This template is the authoritative definition of that file's SHAPE; the duty governing what goes in and when is `roles/goldfish.md` §6 (GF-09-D), stated once there. Together with the `Dispatch: {{TASK_ID}} (goldfish)` commit-trailer line (see Final report below), it is the deterministic authorship/evidence pair for close step 6b and the Critic; `AI-Assisted: true` is only the anonymous assistance marker. Do not put provider/model co-author data, session URLs/IDs, account identifiers, or other private correlation data in a commit.
- **Critic disposition (review-protocol.md §2.1).** Evaluate the trigger matrix before the opening v3 record. Store its closed `pipeline.critic-trigger-input.v1` input and matching `appliedRow` in exactly one of `criticSkip` (`pipeline.critic-skip-decision.v1`, T0/T5) or `criticRequired` (`pipeline.critic-required-decision.v1`, T1-T4). The exact trigger fields are defined by `plugins/pipeline-core/lib/critic-skip-decision.mjs`; a `reason` may explain but never choose the row. Coverage and authorship derive conservative A/G/S signals from the actual commit paths and reject under-declared flags; T0 additionally requires every changed path to be mechanically eligible. After a required Critic finishes, the coordinator replaces `criticRequired` with `criticEvidence` (`pipeline.critic-evidence-reference.v1`) bound to this record's `taskId` and `candidateCommit`, a physical repository-relative path under `evidence/` or `backlog/evidence/`, and the file's SHA-256. A pending `criticRequired` is intentionally a coverage finding.
