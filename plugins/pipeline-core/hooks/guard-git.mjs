...
import { commitMessageFindings, commitTypeFindings, isGitCommit, markerPolicyMode } from "../lib/commit-message-policy.mjs";
...
// `inspection` is declared here, outside the block below, so the GIT-01 block that follows
// (after the GIT-03 block closes) can still read `inspection.message` without recomputing it.
let inspection;
{
...
// ---- GG-22: a commit must not leave an earlier backlog status-flip unreconciled ------------
//
...
if (isGitCommit(tokenizeArgv(cmd))) {
  try {
    ...
    // line ~300) -- any spawnSync error, timeout, or unexpected git output must never itself
    // become a source of an unrecoverable stuck repository.
  }
}
