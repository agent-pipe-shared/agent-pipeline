...
// the new normalized detector. `git status` and a read-only `git log` search whose
// pattern says "push later" are both fully allowed today (no push gate applies);
// the fixed detector must keep
...
// The second command is deliberately non-mutating so independent commit-message
// provenance policy cannot mask this classifier assertion.
check("F1: the shared-normalization fix does not widen push detection to non-push commands", () => {
  const root = fixture();
  for (const command of ["git status", 'git log -1 --format=%s --grep="push later"']) {
