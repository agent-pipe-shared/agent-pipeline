...
check("English runtime projects complete validated queue state", () => {
  ...
  assert.deepEqual(projection.runtime, { humanFacingLanguage: "en", activeDuty: "Coordinator" });
...
check("German runtime renders German without an inferred language", () => {
  const projection = resolveRegroundProjection(outer({
    runtime: { humanFacingLanguage: "de", activeDuty: "Koordinator" },
  }));
...
check("stringify-write-reload preserves semantic projection and state bytes", () => {
  ...
  const value = outer({ runtime: { humanFacingLanguage: "de", activeDuty: "Coordinator" } });
...
for (const [name, value, code] of [
  ...
  ["missing continuity", { schema: "pipeline.state.v0", activeFeature: { id: FEATURE } }, "PCR-CONTINUITY-MISSING"],
  ["mismatched feature", outer({}, { activeFeature: { id: "other" } }), "PCR-FEATURE-MISMATCH"],
]) {
...
check("real compact CLI returns host JSON, exit zero, and leaves state byte-identical", () => {
  ...
  const path = join(root, ".claude", "pipeline-state.json");
  const raw = `${JSON.stringify(outer({ runtime: { humanFacingLanguage: "de", activeDuty: "Coordinator" } }), null, 2)}\n`;
  writeFileSync(path, raw);
