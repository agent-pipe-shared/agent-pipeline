...
- **Critic disposition logging (review-protocol.md §2.1, `pipeline.critic-skip-decision-logged`):** evaluate the trigger matrix before creating the v3 dispatch record. Set exactly one structured decision from `plugins/pipeline-core/lib/critic-skip-decision.mjs`: `criticSkip` for evaluated rows T0/T5, or `criticRequired` for T1-T4. Persist the closed trigger input and its matching `appliedRow`; free-text `reason` is explanatory only. Coverage and authorship independently compare the declaration with the actual commit paths, so under-declared A/G/S work and non-mechanical T0 claims fail. When a required review finishes, the coordinator replaces `criticRequired` with task/candidate/path/digest-bound `criticEvidence`. Until then the coverage gate remains red.
