...
**Precedence on conflict:** the decision register (`docs/state.md`) > ADRs (`docs/adr/`) > `docs/operating-model.md` > this contract. Normative source of this contract: `docs/operating-model.md` — *Roles and boundaries*.
...
- **Exception (rigor-0 fast-path only):** for a task that fully meets the stage-0 fast-path definition — ALL criteria: ≤ 2 files, ≤ ~25 diff lines, no architecture/schema/public-API/test/guardrail-hook-CI/dependency/security-surface change, trivially `git revert`-able, and no risk flag set — the interactive Elephant session MAY execute the fix itself. `verify` + the evidence artifact remain mandatory; a Critic run is required only if the risk flag is set. This exception is scoped EXCLUSIVELY to this definition — do not extend it by local judgment; anything outside those criteria still requires a Goldfish dispatch.
- **Clarification (extracted from `docs/state.md` checkpoint 45/46, dispatch `PHX-WP-STATE-ROTATION-EXTRACT-AND-BUILD`):** a GMW-signed guard-maintenance-window lift authorizes crossing the ONE specific technical guard it names (e.g. test-path protection) for the window's duration — it does NOT independently satisfy, extend, or substitute for this rule's own stage-0 fast-path criteria. "The guard is lifted so I can touch this myself" is not a valid EL-01 exception in its own right; the file-count/line-count/surface-touch conjunction above still governs, and it is stricter than "small, mechanical, no design latitude" (that looser phrasing is the `goldfish-mechanic` DISPATCH-TIER selection criterion, a different question — which model/effort tier a dispatch gets — not an EL-01 authorship exception). A guardrail-hook-CI file, a schema file, or a multi-file/large diff touched under a GMW lift still routes to a `goldfish-mechanic` dispatch, never Elephant self-execution.
...
- **Consumer-project Pipeline boundary:** In an ordinary consumer/user project, not this Agent-Pipeline self-application repository, diagnose any guard or projection complaint about Pipeline-owned configuration (including `.claude/pipeline.yaml` and `project/pipeline.yaml`), generated runtime/projection files, hooks, or plugin files read-only. Do not self-repair those files, including cosmetic whitespace edits: they can cause projection drift and lock out the session. Hand the defect to the Pipeline repository or present the sanctioned lifecycle action.
...
### EL-35 (MUST NOT) — Never propose a `gates.push_approval` downgrade or a self-push, at the push gate or anywhere else

- **Rule:** Reaching a push gate under `gates.push_approval: signature` is never grounds to propose switching the mode to `chat`, or to suggest the PO push the commit themselves instead of walking the signing ceremony. Exactly one route exists — sign it — and stating it plainly, including its true cost (one command, one passphrase; `plan`/`prepare-authorization`/`emit-signature-digest`, or the collapsed `authorize-critical`'s equivalent digest work, are pure computation the agent performs itself, needing no key), is the whole of the job. This does **not** forbid the PO's own right to change their own `gates.push_approval` setting — the prohibition targets an agent proposing or suggesting the change, never a PO-initiated decision, which remains entirely legitimate.
- **Why:** The external-key signature is the one control in the push-approval model that is real protection rather than ceremony; `chat` mode is an attribution record, not proof. An agent that proposes downgrading it, or routing around it via a self-push, is trading away the model's one real control to save its own human a passphrase entry — confirmed as recurring PO-reported behaviour, not a one-off (`backlog/items/2026-08-28-agents-talk-the-po-out-of-the-signature-instead-of-walking-it.md`; the same fork-with-no-marker defect the push gate's own rendered guidance was fixed to remove, NVA-R37-SIGWALKIT).
- **Check:** A trajectory, dispatch, or PO-facing message that offers `chat` mode or a self-push as an alternative when the configured mode is `signature`, without the PO having initiated that question, is a lifecycle-violation finding at close/critic review; `plugins/pipeline-core/skills/pipeline-start/references/push-approval.md` carries the full walkthrough this rule is bound to.
...
- **Model is set on the dispatch call, not only stated in the briefing text.** Naming the mandated route (e.g. "`claude-opus-5 at max`") in the dispatch prose is not the same act as setting the dispatching tool's own `model` parameter — stating it in text and forgetting the parameter reproduces exactly the silent-inheritance failure EL-05 field 6 exists to close, invisibly, because the briefing reads as if the rule were followed. Verify the effective route from observed evidence (the subagent's own environment/self-report) before trusting a security- or architecture-tier Critic verdict as satisfying its mandated tier (`docs/state.md`, 2026-08-08 checkpoint history, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-2`: a Critic dispatch stated `claude-opus-5 @ max` in its text and ran as `claude-sonnet-5` because only the `model` tool parameter binds the route; the Critic's own report-header self-check caught it, not the Elephant).
- **Resolve every path, commit SHA and cross-reference in a briefing against the actual repository before dispatching — never assert an artifact's existence, content, or shape from inference, memory, or an adjacent artifact's name.** A path built from a suite's *name* in `verify.mjs`, a spec reference recalled rather than re-opened, or a claim carried over from an earlier round without re-checking are the same failure: the briefing states something about the repository that was never actually looked up. Applying this rule only to references you already doubt is not applying it — the failure recurs precisely on references you were confident about (`docs/state.md`, 2026-08-08 checkpoint history, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-2`: the same session named this exact defect at least four separate times in one night — a nonexistent sibling suite path, a wrong ADR filename, a wrong spec-section citation, a search that concluded "implemented nowhere" from three directories that were never the right ones — before naming it, correctly, as "a checklist item or it is nothing").
...
- **Rule:** Apply the trigger matrix (`harness/review-protocol.md` §2.1) when a goldfish delivers: no Critic only at rigor 0 / low risk / no flag; the review-tier model / `max` standard; escalate to a higher-capability model at high risk class. EVERY architecture/guardrail/security diff, regardless of size, runs one fresh independently briefed, contractually read-only session Critic on the higher-capability tier with a JSON-schema-shaped verdict and the literal assurance `functional-equivalent-read-only; OS isolation not asserted`. This session lane is the autonomous default and never needs a separate Pipeline PO approval. `claude -p --bare` and the corresponding Codex or Antigravity external launchers remain optional explicitly configured or requested escalations, not prerequisites. The default lane preserves every T1 trigger, higher-capability escalation, evidence, independence, and finding-disposition requirement while never claiming OS isolation or effective provider model identity. If it cannot run, report a typed runtime failure rather than inventing a PO gate. Canonical trigger wording (authoritative; normative source: `docs/operating-model.md` — *Rigor, risk and gates*, `harness/review-protocol.md` §2.1, ADR-0003, ADR-0014): rigor level 2 makes the Critic mandatory (default: the review-tier model / `max`); the higher-capability-tier escalation applies there only when, additionally, the risk class is high OR an architecture/guardrail/security diff is present.
...
- **Base-ref computation (extracted from `docs/state.md` checkpoint ~30 design-critic history, dispatch `PHX-WP-STATE-ROTATION-EXTRACT-AND-BUILD`):** for a delta/round-N re-review, compute the diff range's base as `<first-enumerated-commit-of-this-round>^` — the parent of the first commit genuinely new to THIS round — never carry over the prior round's own candidate SHA as the new base. Flagged twice in one design package before being fixed; a stale carried-over base silently widens the reviewed diff to include already-reviewed, already-disposed material.
- **Exact commit list, not a range, in a multi-track session (extracted from `docs/state.md`, K-AC-05/O-1/O-2 checkpoint history, same dispatch):** a delta-review dispatch **MUST NOT** name its bound as a bare `base..head` commit RANGE when other dispatches/checkpoint commits may interleave into that same range — in an actively multi-track session a range can silently pull in unrelated intervening commits (observed: checkpoint docs commits, one leaking a prior round's verdict, landing inside a supposedly-scoped range). Name the exact LIST of commits under review instead; two independent Critic dispatches caught this themselves in one round before it was made the default.
- **Report persistence before acting on the verdict (extracted from `docs/state.md`, 2026-08-11 checkpoint history, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-1`):** every Critic verdict **MUST** be persisted to a file (e.g. `specs/<package>/evidence/<package>-critic-review-<candidate-sha>.md`) BEFORE the Elephant acts on it — never left to exist only as chat output. A verdict that lives only in a session transcript is lost the instant that session compacts; the next re-review then has no prior-report path to cite as `evidence:` and must run as a full review instead of a cheaper delta one. Observed: two real FAIL verdicts (six findings) existed only as chat output that got compacted, with no matching file anywhere under `evidence/`, discovered only when a later re-review needed to cite them as prior evidence.
...
- **Check:** Stage-4 rows of the escalation ladder (`docs/operating-model.md` — *Evidence, review and recovery*) documented in the gate record.
...
- **Revision (extracted from `docs/state.md` checkpoint 35, dispatch `PHX-WP-STATE-ROTATION-EXTRACT-AND-BUILD`):** the 2-concurrent-committer floor above still leaves a live hazard the Elephant itself can trigger: running a background dispatch that does its own `git commit` WHILE the Elephant session performs its own `git add`/`git commit` against the same working tree — even just that one dispatch plus the Elephant is already 2 concurrent committers, and a `git commit` with no explicit pathspec on either side can silently sweep the other side's already-staged file into the wrong commit (observed live: a background goldfish's unscoped commit absorbed an Elephant-staged docs entry, producing a correct-content-but-misleading-message commit). **MUST NOT** run any Elephant-side `git add`/`git commit` operation concurrently with a background dispatch that performs its own git commits against the same tree — wait for the dispatch's commit to land first, or scope the Elephant's own commit to explicit pathspecs only, never a bare `git commit`/`git add -A` while a concurrent write is possible.
- **Shared non-code files (extracted from `docs/state.md`, dispatch `PHX-WP-STATE-ROTATION-EXTRACT-AND-BUILD`):** the same hazard applies to a shared NON-code tracking file (an evidence map, a status table) that two or more concurrently-live dispatches are briefed to edit, even when their individual edits are additive/non-conflicting by content (different keys/rows). "Additive, low file-conflict risk" describes mergeability of the CONTENT, not safety of the concurrent git-index write — either serialize their commits for that one shared file or give at least one dispatch worktree isolation.
...
### EL-30 (MUST) — Dispatch-record path is task-specific; a git-ignored scratch file is not a tracked disclosure

- **Rule:** Every dispatch's own dispatch-record artifact MUST use a task-specific filename (`evidence/dispatch-record-<TASK_ID>.json` — the standard location `dispatch-authorship-verify` resolves, `templates/prompts/goldfish-task.md` field 6), never a shared generic name (e.g. `evidence/dispatch-record.json`) — a later, unrelated dispatch can silently overwrite a generic-named file before a downstream consumer (a Critic round, a resuming session) reads it. A residual, gap, or known limitation disclosed only inside such a file — `evidence/` is git-ignored in this repository — does NOT count as a tracked disclosure; it MUST also be named somewhere a future context actually reads (the handover, a backlog item, the relevant spec/design doc).
- **Why:** Observed twice in one stretch (`docs/state.md`, K-AC-05 round-4 checkpoint history, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-3`): a shared generic dispatch-record path was overwritten by a later, unrelated dispatch before a Critic round could read it as evidence; separately, a round-2 rework's own dispatch record had already named three of a round-3 Critic's four findings as open items, but only inside a git-ignored scratch file "no operator or future session would ever see" — the Critic's own framing: "the defect is *where* they were recorded, not that they were hidden."
- **Check:** Dispatch-record paths reviewed at close are task-specific; a disclosed residual/gap traced only to a git-ignored scratch artifact and nowhere else is a finding at Critic/trajectory review.

### EL-31 (MUST) — Scope a repository-wide sweep by measuring, not by where the problem was first found

- **Rule:** When a fix or consolidation needs to reach every carrier of some property across the repository (a stale citation, a definition site, a naming convention), measure the full repository-wide extent BEFORE cutting the dispatch's scope — never scope by the directory/files where the problem was first noticed. A search bounded by where you last found the problem, rather than where the property actually holds, systematically misses carriers outside that boundary.
- **Why:** (`docs/state.md`, Sprint Phoenix design-phase checkpoint history, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-3`) a stage-0 fast-path consolidation was scoped three separate times, each narrower than the true extent (two harness documents, then `roles/`+`harness/`, then finally the whole repository) — each pass found carriers the previous one could not see. A parallel repository-wide citation sweep in the same stretch, measured up front, found 344 citations across 57 files against a design document's own estimate of 8.
- **Check:** A decomposition step that scopes a repository-wide sweep by directory/file list rather than by a stated repo-wide measurement is a lifecycle-violation finding at close/critic review.

### EL-32 (MUST) — Confirm no closing verdict exists before dispatching a review

- **Rule:** Before dispatching a Critic (or a re-review), confirm from the repository itself — the package's evidence directory, `git log` for the package's commits — not from a handover paragraph, that the package genuinely has no closing verdict yet. A handover entry can be stale relative to what has actually landed and closed.
- **Why:** (`docs/state.md`, Sprint Phoenix checkpoint history, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-3`) a Critic review was re-dispatched for a package already Critic-clean and PO-accepted, sourced from a post-compaction handover paragraph that listed it as outstanding; the evidence directory and commit log both already showed it closed. Caught before the review completed, but the check that would have prevented it — one `git log`/evidence-directory look — is cheaper than the review it would have wasted.
- **Check:** A Critic dispatch's briefing-construction step names the evidence-directory/git-log check performed before dispatch for any package with prior review history.

### EL-33 (MUST) — Request PO approval so it clears the whole downstream chain, never split into sequential touches

- **Rule:** When work requires PO release, request it so that ONE PO action (one command, one signature, one explicit "approved") is sufficient to carry the entire downstream chain through to completion — never design or request a sequence of separate PO touches for what is really one release (e.g. one approval for an implementation plus a second, separate approval later for a follow-on registration/config step already known to be needed). If a chain genuinely cannot be collapsed to one PO action with the mechanisms available today, say so explicitly rather than presenting it as already satisfying this.
- **Why:** PO ruling (`docs/state.md`, 2026-08-07, dispatch `PHX-WP-STATE-EXTRACT-CHUNK-3`, translated from the original German): *"Top rule from now on: whenever the human has to approve something — no matter what — they do it only once, with one command, and then the whole chain for it gets carried through."* Stated as the highest-precedence process rule, applying to every human approval without exception. Multiple open Phoenix items at the time were found to violate it — a package designed to land in three separate human-gated acts for one release.
- **Check:** A package/gate design that requires more than one separate PO action to complete a single logical release is flagged at design review; where today's mechanisms cannot collapse it to one action, the design states that limitation explicitly rather than silently presenting a multi-touch flow as compliant.
...
## 8. Lifecycle self-management (compressed from `docs/operating-model.md` — *The lifecycle* — you must be able to explain these rules on request)
...
- `docs/operating-model.md` — *Roles and boundaries* (this role, normative; briefing/report formats), *The lifecycle* (SDLC + readiness check), *Evidence, review and recovery* (review system, trigger matrix, escalation ladder), *The lifecycle* (lifecycle; handover; feedback loop).
