...
- **`GG-03` has a second, stronger route that needs no phrase at all ([ADR-0061](../docs/adr/0061-uniform-human-approval-ceremony.md)).** When a `git push` is matched by `GG-03` **and by no other rule**, and `authorizeRecordedPush` verifies a recorded approval for exactly this candidate commit, remote and destination ref against the committed trust anchor, the guard admits it with no arming, no token, and no typed `OVERRIDE GG-03` — writing an audit entry naming the authorization it relied on. A denial names the returned `PUSH-PROOF-*` code. The reason is ADR-0061's disposal test: the typed phrase prevents no agent behaviour the signature does not already prevent, and the signature is strictly stronger — per-commit, per-destination, verified against a committed anchor, and impossible for the agent to produce. `GG-03` blocks exactly as before whenever no such approval exists, which is the unattended-agent case it was built for; a `--force` or `+refspec` push also matches `GG-01`/`GG-02` and therefore still blocks, approval or not. **The push must write out its destination ref** (`HEAD:refs/heads/<branch>` or `<sha>:refs/heads/<branch>`) — a command that does not name a ref cannot be matched against an attestation without guessing.
