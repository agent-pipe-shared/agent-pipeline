...
> Audience: every agent role touching a project that declares a manifest `release` section (`project/pipeline.yaml`, else `.claude/pipeline.yaml`), or any hook/lib code enforcing it, in any pipeline-bound project and in this repo (self-application).
