...
- **Not yet done, explicitly carried forward:** dispatch `NVA-BL-40`'s fix-round (small, QG-07, add a committed two-parent-HEAD-rejection test replacing the deleted throwaway-clone proof) and `NVA-BL-42`'s fix-round (F1: restore the `critic.md` content-integrity pin correspondence, or update it with disclosure; F2: real verify.mjs evidence) once `NVA-BL-20-FIX` lands; dispatch `NVA-BL-34`'s own Critic review; report the three-FAIL correction to the PO (owed regardless of gate status — a material correction to the last-reported "8 landed clean," not something to fold silently into a later summary).
