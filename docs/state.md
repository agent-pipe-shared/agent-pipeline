...
3. Draft ADRs for #1 (A2/`#29` selected-sandbox launcher), #2 (A4/`#12`,`#14` execution-plane executor), #3 (A6/A6R release-preflight consent, modeled 1:1 on ADR-0056's signature/chat shape) — dispatch as design-tier (opus) subagent work, not this session's own model, per MP-01/MP-18. Then dispatch the actual production implementation (goldfish-deep, since this is genuine in-task design/security latitude) once each ADR is drafted and can be marked "accepted" citing the PO's chat instruction as basis (the established pattern from ADR-0056/ADR-0058).
