...
2. If more AFK time remains before the next hard gate: the two deferred-to-design-round architectural items, or the GMW bug fix (needs isolated dispatch — the fix file is kernel-protected), or picking at `#98`'s remaining R3/R4/R6 gaps (unrelated to ADR-0064, per its own Scope decision) are the next candidates, roughly in that order of tractability under a no-push, no-PO-touch constraint.
3. **No push.** 17 local commits since the last push (`2eb4466c`) — see the full sequence above. Stay local until the PO clears the next push explicitly.
