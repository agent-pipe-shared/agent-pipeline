...
Backlog now **236/295 closed, 52 open, 7 in_progress** (live count, refreshed after this block's backlog-cleanup pass — `dispatch-record-does-not-bind-to-its-commit`, `po-key-trust-anchor-onboarding`, `wave5-scope-categorization-missed-triage-level-sprint-deferrals`, `staleness-check-unsettled-top-level-await-warning`, `installing-consumer-is-never-asked-any-setup-decision` all closed this block).
