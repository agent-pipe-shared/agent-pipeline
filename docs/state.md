...
**Next planned step:** continue Wave 5 with round 3 drawn from the corrected (post-exclusion) remainder — of the ~37 actionable candidates identified so far, 6 are landed/closed (NVA-W5-02/03/04/05/07/08) and 2 more have real but partial progress (NVA-W5-01 blocked on a TP-ceremony; NVA-W5-06 was mis-scoped, belongs to Alfred). Prioritize well-scoped non-TP-protected items first, same as rounds 1-2's selection strategy; several remaining candidates (`unregistered-suite-is-red-and-invisible-to-verify`, `two-guards-block-an-unrelated-file-via-substring-name-matching` part B) are blocked on out-of-session PO/TP-ceremony action and are not dispatchable at all today.
