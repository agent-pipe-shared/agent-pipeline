...
**Last updated:** 2026-08-19 — **Full-backlog code-first verification completed, Wave 4 corrected to 12/17, Wave 5 scoped and round 1 in flight.** After the stale "15/17 Wave-4-closed" claim was caught and corrected (see `docs/state-archive/` for that correction's own entry), the PO directed a genuine code-first check of the ENTIRE open backlog (98 open/in_progress items, not just Wave 4) rather than trusting status text — a 14-agent read-only Workflow (task `wdyd7rk9g`) individually verified every item against current code. Result: 22 verdicts `resolved-in-code` (19 closed immediately, commits `9ae28dad`/`1eb05885`; 1 more closed after publishing its rationale, commit `188a84b0`; 2 stay open for a stated deliberate reason — see the "Vier Sonderfälle" artifact linked below), 21 belong to another already-scheduled sprint (Alfred/Nightwing/Phoenix — explicit deferral text in their own Triage, cross-checked against sprint-name mentions in the verification evidence), 54 are genuinely open and unscheduled (**Wave 5's real scope**), 1 is unable-to-determine from static code (needs a live fresh-directory reproduction). Of the 19+1 closed-in-code items, **5 were original Wave-4 (17 Rune) items** (`transactional-readbacks-not-status-cascades`, `capability-first-advisor-critic-dispatch`, `compaction-stable-bootstrap-lease`, `per-phase-progress-message-budget`, `explicit-final-acceptance-gate`), so **Wave 4 now stands at 12/17 closed**, not 7/17 or the earlier-wrong 15/17. Remaining Wave-4-open (5): `lossless-pre-restart-checkpoint`, `intake-values-restart-resilient-immediately`, `fresh-repo-onboarding-intake-first-transaction` (all three subsumed into one unbuilt "atomic bootstrap-from-intake coordinator", design-only at `specs/wave4-onboarding-coordinator/design.md`), `self-healing-local-cleanup-recovery` (partial, F4/F5 + `quarantine-private-receipt` test coverage remain), `po-key-trust-anchor-onboarding` (deliberately scope-reduced/deferred per its own 2026-08-19 note). A detailed HTML breakdown of the 3 deliberately-still-open + 1 unable-to-determine items (procedure/impact/options/recommendation per item) was published as an Artifact: https://claude.ai/code/artifact/f4524bb1-87ba-45f7-a838-e41a74c1c91b.

**Local checkpoint candidate stamped: `6449cdfb`** (manifest `+{claude,codex}.20260819053503.b543069`; content commit `33a8ec5e37d89a734fc7236b66bdad0515bc5b96`). This is still a checkpoint, not the final Nova A 0.6.0 candidate — the manifest has not been re-stamped since the 20-item closure batch above; a fresh stamp is due once Wave 5 round 1 (below) lands. Full diagnosis of the 6 verify defects found on the pre-fix tree (`79007f73`) and the standing marketplace-mirror exception are unchanged from the prior entry (archived).

**Wave 5 round 1 dispatched (in flight, task `wqu3ioowr`):** 6 well-scoped, non-TP-protected fixes from the 54-item Wave-5 pool, each a genuine Goldfish-implementor dispatch (6-field briefing, worktree-isolated, self-heal instruction included since `refs/remotes/origin/HEAD` is still stale in this checkout): `guard-dispatch-has-no-workflow-tool-awareness` (NVA-W5-01), `vendored-dispatch-templates-drift-from-canon` (NVA-W5-02), `onboarding-continuity-assumes-calibration-handover-is-always-a-plain-string` (NVA-W5-03, remaining sibling call site), `reconcile-backlog-ledger-evidence-commit-predates-referenced-file` (NVA-W5-04), `backlog-plan-writers-skip-drift-classification` (NVA-W5-05), `lifecycle-guard-does-not-know-the-human-signing-commands` (NVA-W5-06). Selected specifically for having NO TP-protected files in their fix path (confirmed against `project/guard-config.json`'s pattern list) so no signature ceremony blocks them mid-dispatch. Results not yet landed as of this entry — the remaining ~48 Wave-5 items (many needing a real design pass, several blocked on TP-protected files needing a signed ceremony, or on `harness/scripts/verify.mjs` registration) are the next rounds.
