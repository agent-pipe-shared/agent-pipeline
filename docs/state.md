...
**What remains open, named explicitly:** ADR-0065 candidate (c) (bulk narrowing of further suites, ordered by `durationMs`) — not started. `#98` R4's real-data exercise — confirmed blocked on Codex-transport infrastructure this session cannot reach. The two large architectural backlog items (`every-gate-binds-the-whole-tree`, `nothing-connects-an-acceptance-criterion`) — deferred to a dedicated design round, per their own Triage. `NVA-VERIFYDUR-1`'s remaining one-line change and the marketplace-copy refresh — both genuinely PO-gated (the two-command refresh named exactly in the earlier `2026-08-17` entry). The GMW/HGO reconcile manual-copy item — deferred, owner `pipeline`, due `2026-08-30`. No push.
