...
**Current HEAD: `cf0662d0`.** No push. No other PO-requiring action beyond: the plugin-dedup operational-choice question (still open, unanswered), the marketplace-refresh retry confirmation, and the HGO TP-3 signature retry for F-A (neither of the latter two blocking).
