...
Backlog now **240/297 closed, 50 open, 7 in_progress** (live count, refreshed after this file's own 2026-08-19 afternoon block — coordinator step 5 landed, the Phoenix migration item and 2 stale-status items reconciled).
...
- **F1 stays open**, filed as a correction to `backlog/items/2026-08-19-backlog-status-drifts-from-code-across-compaction-with-no-hardening.md` (piece 2b) — needs its own fresh signed TP-3 HGO ceremony (a PO `sign-intent` action outside the session) to register the actual checker; not attempted this block.
