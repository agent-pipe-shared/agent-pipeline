...
- **Two small items carried to next session, not worth their own dispatch:** (1) the five `result.txt` runtime-artifact files committed under `plugins/pipeline-core/scripts/fixtures/nova-benchmark/workloads/<class>/` (noted in the NVA-A8-RUNNER bullet below) are still tracked and untidied. (2) Full Verify has not been re-run since the three NVA-A8 cherry-picks (`028197b6`/`9fb714ae`/`86659980`) landed on top of the last clean 266/267 pass — that pass predates those commits, so their blast radius beyond their own 11/11 unit suite is unconfirmed.
