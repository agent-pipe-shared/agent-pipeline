...
**Final candidate, this time for real: `a4cc2d84`.** Test-only file change, independently re-verified directly (4/4 + sibling 15/15 unaffected) rather than a fresh full 269-suite Verify run at this exact commit — after ten-plus full Verify+Security cycles already run this block, all green, on a change class (adding one more disposable-fixture test case) that full Verify would not meaningfully re-validate beyond what direct re-execution already confirmed. This is a deliberate, disclosed exception to this block's own re-stamping discipline, not an oversight — the next full Verify run, whenever it next happens, binds at this commit or later.

**Closing this AFK block here.** R0 wiring, ADR-0064/0065 acceptance, and the marketplace refresh are the three remaining threads, none reachable further without either genuine additional design investigation (R0) or the PO. Everything else identified across this block — Nova A's stale-status corrections, two real bug fixes, ADR-0064 and ADR-0065 both proposed, three R6 fixture increments, one backlog item filed from a live cross-repo finding — is committed, independently verified, and captured in this file. No push. No PO-touching action taken.
