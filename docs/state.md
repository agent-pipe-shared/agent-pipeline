...
- **Major #12/#14 scope correction: the production executor largely already exists.** `docs/adr/0048-local-goldfish-supervisor.md` (accepted by PO instruction, 2026-07-26, three days after ADR-0044 named "Local Goldfish pool" as the selected future option) already authorized and — per its own exact-manifest — implemented a real same-host supervisor: independent Git clones, real child processes, a production Codex `exec` adapter (`--strict-config --sandbox workspace-write --ephemeral --json`), capacity/cleanup/cancellation, ~2,600 lines across `plugins/pipeline-core/lib/local-worker-supervisor.mjs` (1354 lines) + its CLI + both test files. ADR-0062's `#12`/`#14` work is therefore mostly WIRING this existing supervisor's results into `scheduling-lifecycle.mjs`'s `composeSchedulingLifecycle`/`terminalOutcomes`, not building an executor from nothing — a much smaller, better-specified job than originally scoped. Confirmed via a repo-wide `rg` that `#29`'s probe-start/probe-success semantics (`reduceSelectedSandboxDisposition`) are NOT covered by this supervisor at all (zero matches) — `#29` remains its own real wiring task, separate from `#12`/`#14`, though it may reuse the supervisor's process-launch machinery as a building block.
...
- **NVA-A8-RUNNER (`plugins/pipeline-core/scripts/nova-a8-benchmark-runner.mjs`, new fixture workloads) — dispatched, in progress:** implements decision (2) above. Five new deterministic workload fixtures under `plugins/pipeline-core/scripts/fixtures/nova-benchmark/workloads/<class>/` (sibling to, never replacing, the frozen hash-pinned `<class>.json` markers) — each writes a fixed result to `result.txt` (never stdout, to stay robust against `claude -p` conversational variance), fully deterministic, no LLM reasoning in the measured task itself. Runner drives each through both `"serial"` (direct Node subprocess) and `"native"` (real `claude -p`, reusing `critic-bare.mjs`'s established non-interactive invocation shape) routes, 1 warmup + 5 measured reps each (schema-pinned `measured: 5`, not changed), real `hrtime`-based timing, real candidate commit/tree binding, and produces one real sealed `pipeline.multi-cli-benchmark.v1` record — explicitly instructed to report the real `recommendation` value honestly (including `"no-recommendation"`) rather than engineer a forced positive result.
