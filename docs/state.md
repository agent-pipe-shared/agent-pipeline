...
**Not yet done, in priority order:** (1) `NVA-W5-COORD-STEP6-1`'s DoD tests finish → independently verify (never trust the returned result text) → cherry-pick → re-run the 5 named DoD suites on trunk. (2) One more fresh full Verify against the fully quiesced tree once step 6 lands. (3) Stamp a new **local** Nova A 0.6.0 candidate for functional/technical testing ("fachlicher Test") — **PO explicitly clarified (2026-08-19) this stamp does NOT need the final Critic gate first**. (4) Present the backlog triage block (above) to the PO for individual decisions — do not dispatch unilaterally. (5) Relaunch the final T1 Critic gate review (diff range `83f564df..HEAD`, opus/max per `routing.duties.critic_high_risk.claude`) only once ready to designate a candidate as final — the actual last piece per the PO's original standing instruction ("...inkl finalem critic"), NOT a blocking prerequisite for step (3)'s local stamp.
