...
ADR-0070 `pipeline.local-supervisor-state.v3` and B1-C
...
The supervisor proceeds only after exact ADR-0070 `ready/noop` readback for the
...
read back under the ADR-0070 root. It binds:
...
`<ADR-0070-root>/workspaces/<lease-id>`
...
ADR-0070 and B1-C source/schema files are imported authorities and are not
...
- Extend ADR-0070 v3 or B1-C v1 in place: rejected; both accepted shapes remain
