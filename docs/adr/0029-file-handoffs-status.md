...
4. **Gates check EVIDENCE FRESHNESS, never compute themselves** (10-second hook budget): the push-gate hook accepts only one standalone, explicit repository/source push, resolves that source to a commit OID, then reads `evidence/verify-latest.json` and (if declared) `evidence/security-latest.json` from that repository. It checks only `exitCode === 0` AND `commit === pushed source OID`; approval binds the same OID. Ambiguous repo/ref/bulk/bundled pushes fail closed once the gate is active. The hook itself NEVER runs `verify` or the security scan.
...
4. **Gates prüfen EVIDENZ-FRISCHE, nie selbst rechnen** (10-Sekunden-Hook-Budget): Der Push-Gate-Hook akzeptiert nur genau einen eigenständigen, expliziten Repository-/Source-Push, löst dessen Source zum Commit-OID auf und liest dann `evidence/verify-latest.json` sowie gegebenenfalls `evidence/security-latest.json` aus genau diesem Repository. Geprüft wird nur `exitCode === 0` UND `commit === gepushter Source-OID`; die Freigabe bindet denselben OID. Mehrdeutige Repo-/Ref-/Bulk-/Bundle-Pushes blockieren bei aktivem Gate. Der Hook führt selbst NIE `verify` oder den Security-Scan aus.
