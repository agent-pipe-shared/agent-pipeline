...
**Release/deploy carve-out (2026-07-12, sdlc-release-deploy):** A deploy-triggering push (a tag/branch push that fires `deploy:test`/`deploy:prod` under an active Release/Promotion phase, §3.5) is NOT covered by this repo's standing approval — it additionally needs a recorded `deployApproval` (the guard-push carve-out) before the push guard lets it through. Detail: [ADR-0033](0033-release-promotion-phase.md) (Release/Promotion phase), [ADR-0034](0034-deploy-precedence-central-vs-project.md) (central deploy-policy precedence).
...
**Release-/Deploy-Carve-out (2026-07-12, sdlc-release-deploy):** Ein Deploy-auslösender Push (ein Tag-/Branch-Push, der `deploy:test`/`deploy:prod` unter einer aktiven Release/Promotion-Phase auslöst, §3.5) ist von der Standing-Approval dieses Repos NICHT gedeckt — er braucht zusätzlich eine verbuchte `deployApproval` (der Guard-Push-Carve-out), bevor das Push-Gate ihn durchlässt. Detail: [ADR-0033](0033-release-promotion-phase.md) (Release/Promotion-Phase), [ADR-0034](0034-deploy-precedence-central-vs-project.md) (Präzedenz zentrale Deploy-Policy).
