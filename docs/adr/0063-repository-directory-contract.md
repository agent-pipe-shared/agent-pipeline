...
| Evidence artifacts (Verify receipts, dispatch records, Critic verdicts, closure evidence cited by backlog frontmatter) | `evidence/` (repo root, machine-generated Verify status) or `backlog/evidence/` / `specs/*/evidence/` (durable citation targets) — **never both meanings in the same directory** | root `evidence/` is ignored (regenerated per Verify run); `backlog/evidence/` and `specs/*/evidence/` are tracked | scratch/probe scripts, commit-message drafts, anything not cited by a gate or a backlog `closure_evidence` field |
