...
## Historical Phase 2.6 direct-route extension (2026-07-15)
...
This historical v1 direct-route binding used an unresolved `terra` alias for
some Codex duties. P3B v2 supersedes that selector for its execution cells:
`Terra` now means the exact **requested** Codex model ID `gpt-5.6-terra` at the
registered effort; it is not a claim about the effective provider model.
Fable duties resolve to `gpt-5.6-sol` at the same effort tier, while Claude
assignments remain unchanged. A route or effective model is attested only by a
valid same-dispatch receipt with its required bindings. Agent self-report and a
missing or invalid receipt are insufficient; the route stays unresolved,
unavailable, or unbound according to the relevant contract.
