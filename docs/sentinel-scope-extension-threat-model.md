...
| A versionless Codex manifest is accepted from a project, private lock, environment, CLI input, or a different installed plugin. | The Codex wrapper first runs the fixed, closed-schema host plugin-list command to derive the selected source root; `observeCodexPublicCoreIdentity` independently re-runs it to obtain the version and requires its source root to match the requested one. Both calls use the trusted absolute Codex executable, never `PATH`. Each requires exactly one enabled selected Pipeline entry. A local SHA-phase marketplace is accepted only when it is exactly the canonical repository parent of that selected source; otherwise only a credential-free HTTPS Git marketplace is accepted. Generic observation still requires manifest versions; snapshot, manifest-digest, content-digest, candidate, and private-lock checks remain unchanged. | Public-observation and Codex-activation regressions reject caller-supplied/unavailable host versions, malformed/non-selected list entries, root/version mismatch, arbitrary local marketplaces, and snapshot drift; Full Verify and Security bind the candidate. | The installed Codex host is a trusted local prerequisite. An unavailable, changed, or malformed host response is fail-closed; it cannot fall back to project or private configuration. |
...
| A runner-neutral cutover moves calibration, guard configuration, guard audit, or State while a direct consumer still trusts a legacy path. | One coherent authority resolver selects manifest, State, calibration, guard configuration, and guard audit together. Safety consumers protect both State aliases, Security binds the selected manifest/policy bytes, mixed layers fail closed, and migration refuses a non-null machine-local cleanup handle before creating transaction bytes. | `project-authority.test.mjs`, `project-authority-migration.test.mjs`, `workflow-writer-preflight.test.mjs`, `security-scan.test.mjs`, and Full Verify cover neutral, legacy, mixed, blocked-cleanup, rollback, and exact-policy evidence. | The legacy layer remains read-only compatibility input during rollout. It is not publication authority and cannot authorize a neutral write or Security claim. |
