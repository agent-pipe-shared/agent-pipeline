...
| A versionless Codex manifest is accepted from a project, private lock, environment, CLI input, or a different installed plugin. | Only `observeCodexPublicCoreIdentity` runs the fixed, closed-schema `codex plugin list --marketplace agent-pipeline --json` command. It requires exactly one enabled selected Pipeline entry, binds its physical source root to the requested source, and uses its observed version only for the versionless-manifest comparison. Generic observation still requires manifest versions; snapshot, manifest-digest, content-digest, candidate, and private-lock checks remain unchanged. | Public-observation and Codex-activation regressions reject caller-supplied/unavailable host versions, malformed/non-selected list entries, root/version mismatch, and snapshot drift; Full Verify and Security bind the candidate. | The installed Codex host is a trusted local prerequisite. An unavailable, changed, or malformed host response is fail-closed; it cannot fall back to project or private configuration. |
