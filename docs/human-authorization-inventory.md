...
|---|---|---|---|---|
| Push <!-- inventory-id: push-signature --> | `git push` of the candidate commit to a remote/ref | `push` | `gates.push_approval` in `pipeline.user.yaml` (`signature`\|`chat`, default `signature`); ADR-0056 | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`approve-push`, ~L7123-7257), `lib/critical-action-authorization.mjs`, `hooks/guard-push.mjs` for consumption; the PO-side ceremony is `po-human-approval.mjs authorize-critical` (kind `push`, ADR-0061 port) or, in `chat` mode, `chat-gate-ceremony.mjs`'s attended-TTY confirmation |
| Deploy <!-- inventory-id: deploy-signature --> | Consuming a deploy approval for `{artifact, environment}` | `deploy` | `project/critical-human-proof.json` `requiredKinds`/waiver only (no `pipeline.user.yaml` key); ADR-0055/0056 | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`approve-deploy`/`consume-deploy`/`clear-deploy`, ~L7387-7507), `lib/critical-action-authorization.mjs` |
| Publication <!-- inventory-id: publication-signature --> | `publication-approve` (a State→publication projection transaction) | `publication` | `requiredKinds`/waiver only (no `pipeline.user.yaml` key); **not** rebuilt onto ADR-0056 Decision 7's push shape — ADR-0056 Follow-up states outright "the two now differ in shape, and one shape would be better than two" | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`runPublicationCommand`, ~L2618-2802) |
| Feature-package reconcile <!-- inventory-id: feature-package-reconcile-signature --> | `feature-package-reconcile` (cross-repository package reconciliation) | `feature-package-reconcile` | `gates.reconcile_approval` in `pipeline.user.yaml`, same shape as push (ADR-0056 2026-08-11 follow-up) | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`defaultFeaturePackageReconcileApproval`, ~L6259-6284); this kind is in `ALWAYS_REQUIRED_KINDS` — always demanded, not gated by `requiredKinds` membership |
| Release preflight consent <!-- inventory-id: release-preflight-signature --> | Consenting that a release attempt for the candidate may be prepared and taken to the independently operated final gates — explicitly not a release, not a publication authorization, and not a pass of any of those gates (GF-105/ADR-0064 Decision 4) | `release-preflight` | Same shared contract; reachable via the direct `prepare-critical`/`approve-critical`/`verify-critical`/`authorize-critical` command family (`CRITICAL_COMMAND_KINDS`), same as `push`/`deploy`/`publication`/`feature-package-reconcile` | `plugins/pipeline-core/scripts/po-human-approval.mjs` (`resolveSubjectPreimage` ~L745-761, `releasePreflightConfirmationLines` ~L798-814), `plugins/pipeline-core/lib/critical-action-approval-request.mjs` (`CRITICAL_ACTION_KINDS`) |
| Governance fork disposition <!-- inventory-id: governance-fork-disposition --> | Disposing of a forked-subagent scope-creep finding | `governance-fork-disposition` | Same shared contract | `plugins/pipeline-core/scripts/po-human-approval.mjs` (`runForkDispositionApproval`, ~L426-484) |
| Critical action (generic H-AC ceremony) <!-- inventory-id: critical-action-family --> | The ADR-0061-ported "authorize-critical" flow: an agent-prepared request over one of the `CRITICAL_ACTION_KINDS` (`push`, `deploy`, `publication`, `release-preflight`, `governance-fork-disposition`, `feature-package-reconcile`), authorized externally | `critical-action` (outer intent) wrapping the actual action `kind` + `subjectSha256` in the signed subject | Same shared contract | `plugins/pipeline-core/lib/critical-action-approval-request.mjs`, `plugins/pipeline-core/scripts/po-human-approval.mjs` (`criticalApprovalRequest`, ~L223-249) |
| Guard Maintenance Window (GMW) install/lift <!-- inventory-id: guard-maintenance-window --> | Time-boxed PO-signed exception letting a self-protecting guard rule (GS-6, protected test paths) honor an additional allow-path | `guard-lift` | Same shared contract, trust-anchor default `project/critical-human-proof.json`; signed via `po-human-approval.mjs sign-intent` against a bare intent digest | `plugins/pipeline-core/lib/guard-maintenance-window.mjs`, `guard-maintenance-window.mjs install`; ADR-0058; `docs/guard-maintenance-window-threat-model.md`, `docs/po-human-approval.md` §"Signing a bare intent digest" |
| Human Guard Override (HGO), signed route <!-- inventory-id: human-guard-override-signature --> | Signed admission past a PreToolUse guard denial for one exact request/plan pair, via `guard-human-override.mjs plan`/`prepare-authorization`/`emit-signature-digest`/`authorize-by-signature` | `guard-override` | Same shared contract (default trust anchor same file); the `activate`/chat route also exists alongside this signed one (ADR-0059 Decision 1) | `plugins/pipeline-core/lib/human-guard-override.mjs` (~L1796-1825), `plugins/pipeline-core/scripts/guard-human-override.mjs` |
| Threat model approval (Cyborg CYB-4) <!-- inventory-id: threat-model-approval --> | PO decision on a threat-model boundary (`approved`/other) | `threat-model` | Same shared contract — the original adapter this whole contract generalized from | `plugins/pipeline-core/lib/threat-model.mjs`, `lib/threat-model-approval-request.mjs` |
| Security authority proof <!-- inventory-id: security-authority-proof --> | A recorded security-authority decision | `security-authority` | Same shared contract | `plugins/pipeline-core/lib/security-authority-proof.mjs` |
...
|---|---|---|---|
| Plan/PRD approval (`approve-plan`) <!-- inventory-id: plan-prd-approval --> | Approving a submitted plan, moving the feature out of `awaiting-approval` | Repository-scoped **PO-gate-authority binding**: `poGateAuthority`/`validatePoGateAuthorityForRepository` plus `poGateProfile`/`validatePoGateProfileForRepository` check the authority's bound `planPath`/`planSha256`/`specSha256` against the submitted plan. `approve-plan` deliberately consumes the preceding Bootstrap PRD/Spec acknowledgement rather than asking for a second proof. That acknowledgement is selected by `gates.human_approval`: `signature` verifies the exact signed `bootstrap-plan-acknowledgement` receipt; `chat` records the explicitly selected attributed acknowledgement. The final plan transition itself stays off the Ed25519 contract, so one reviewed design decision does not create two signing ceremonies. | `plugins/pipeline-core/scripts/project-onboarding-v3.mjs` (`bootstrap-acknowledge-plan`/`bootstrap-acknowledge-apply`/`bootstrap-acknowledge-chat-apply`), `plugins/pipeline-core/lib/onboarding-continuity.mjs`, `plugins/pipeline-core/scripts/pipeline-state.mjs` (`approve-plan`, ~L6869-6939) |
| PO-authority rebind (`po-authority-rebind-plan`/`-apply`) <!-- inventory-id: po-authority-rebind --> | Rebinding the PO-gate-authority record itself to a new manifest/profile (the mechanism `approve-plan` depends on) | Amendment-record-gated transaction (recent hardening: `feat(pipeline-core): require amendment record for immutable manifest rebinds`); not the Ed25519 proof contract | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`runPoAuthorityRebindCommand`, ~L5224-5260 and helpers ~L4668-5220) |
| PO-authority decision (`po-authority-decision-plan`/`-select`/`-apply`) <!-- inventory-id: po-authority-decision --> | Recording a PO decision that revises authority (a decision-selection transaction distinct from a manifest rebind) | Same PO-gate-authority family as above, own plan/select/apply transaction, not the Ed25519 proof contract | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`runPoAuthorityDecisionCommand`, ~L5261-5401 and `buildPoAuthorityDecisionPlan` ~L4901-5075) |
| Push (`chat` mode) <!-- inventory-id: push-chat-mode --> | Same push action as above, when `gates.push_approval: chat` | Explicit, self-declaring **attribution record** — a human runs the approval command in-session; ADR-0056 states plainly this is "an attribution record, not proof of a human" | `plugins/pipeline-core/scripts/pipeline-state.mjs` (`approve-push`), `lib/critical-human-proof-policy.mjs` |
| Deploy/Feature-package-reconcile (waived) <!-- inventory-id: deploy-reconcile-waiver --> | Same actions as above, when a kind carries an ADR-0055 waiver in `project/critical-human-proof.json` | Recorded waiver (`kind` + `reason`), a committed, diffable, attributable act of standing the cryptographic check down — not proof | `plugins/pipeline-core/lib/critical-human-proof-policy.mjs` |
...
`check-auth-gate-inventory-drift.mjs`. It classifies every row in the two
inventory tables above. `registered` means the closed catalog calls the
existing source builder; `legacy-renderer` means the current producer remains
authoritative until a later migration can preserve its exact action shape;
`abstract-family` is metadata which cannot be run directly; and `excluded`
means no terminal action exists. These are implementation dispositions, not
new approval gates.
...
| `release-preflight-signature` | `legacy-renderer` | — |
| `governance-fork-disposition` | `legacy-renderer` | — |
| `critical-action-family` | `abstract-family` | — |
| `guard-maintenance-window` | `legacy-renderer` | — |
| `human-guard-override-signature` | `legacy-renderer` | — |
| `threat-model-approval` | `legacy-renderer` | — |
| `security-authority-proof` | `legacy-renderer` | — |
| `plan-prd-approval` | `legacy-renderer` | — |
| `po-authority-rebind` | `legacy-renderer` | — |
| `po-authority-decision` | `legacy-renderer` | — |
| `push-chat-mode` | `legacy-renderer` | — |
...
contains rendering data but no structured execution-boundary fields, so the
catalog preserves that absence as `boundary: null`. Adding boundary fields is
a producer migration and cannot be inferred in this inventory.
