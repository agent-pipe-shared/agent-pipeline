...
| Release preflight consent | Consenting that a release attempt for the candidate may be prepared and taken to the independently operated final gates — explicitly not a release, not a publication authorization, and not a pass of any of those gates (GF-105/ADR-0064 Decision 4) | `release-preflight` | Same shared contract; reachable via the direct `prepare-critical`/`approve-critical`/`verify-critical`/`authorize-critical` command family (`CRITICAL_COMMAND_KINDS`), same as `push`/`deploy`/`publication`/`feature-package-reconcile` | `plugins/pipeline-core/scripts/po-human-approval.mjs` (`resolveSubjectPreimage` ~L745-761, `releasePreflightConfirmationLines` ~L798-814), `plugins/pipeline-core/lib/critical-action-approval-request.mjs` (`CRITICAL_ACTION_KINDS`) |
...
| Critical action (generic H-AC ceremony) | The ADR-0061-ported "authorize-critical" flow: an agent-prepared request over one of the `CRITICAL_ACTION_KINDS` (`push`, `deploy`, `publication`, `release-preflight`, `governance-fork-disposition`, `feature-package-reconcile`), authorized externally | `critical-action` (outer intent) wrapping the actual action `kind` + `subjectSha256` in the signed subject | Same shared contract | `plugins/pipeline-core/lib/critical-action-approval-request.mjs`, `plugins/pipeline-core/scripts/po-human-approval.mjs` (`criticalApprovalRequest`, ~L223-249) |
