...
| Portable stream writer lock | Per-stream filesystem coordination | A live lock blocks writers; a POSIX advisory acquisition guard serializes every lock create/recovery. It is released by the kernel on a crashed owner, so a dead `.lock` is unlinked only while no competing acquirer can replace it. |
...
| Two writers recover the same dead lock while one has already acquired a replacement | A POSIX advisory acquisition guard covers stale-owner observation, unlink, and replacement creation for every writer. The kernel releases a dead recovery owner's guard, while a live owner makes the next writer fail closed. | Only the guard holder can reclaim the dead pathname; a competing writer fails closed or observes the later live lock, so no recovery can delete a replacement writer lock. |
