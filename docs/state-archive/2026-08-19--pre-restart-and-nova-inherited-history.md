# Archived handover history — 2026-08-19 rotation (pre-restart and Nova-inherited history)

> Archived under [ADR-0064](../adr/0064-handover-rotation-extraction-archive-hard-size-gate.md)
> Decision 3 (one self-contained file per rotation event). This file is a
> **verbatim, byte-for-byte** copy of a contiguous range of the live
> `docs/state.md` at the moment of rotation — nothing summarized,
> nothing paraphrased, nothing deleted. Every section below still carries
> the `pipeline.handover-rotation-extraction-ack.v2` marker it had in the
> live file; the marker is harmless here and left in place rather than
> stripped, so this archive file remains a faithful copy rather than a
> re-edited one.

## Provenance

- **Source file:** `docs/state.md` (this repository, branch `sprint_phoenix`).
- **Original line range archived:** lines 4977–19155 (inclusive) of
  `docs/state.md` as it stood immediately before this rotation — the entire
  remainder of the file below the 54 sections kept in the live head.
- **Rotation date:** 2026-08-19.
- **PO authorization:** PO in-session decision, 2026-08-19: archive
  candidates A (Nova-inherited history), B (Phoenix pre-restart history), C
  (oldest era + open-items tail) — all archived, none deleted.
- **Extraction-pass summary (ADR-0064/ADR-0060 safety precondition for this
  range):** the two prior extraction rounds (dispatches
  `PHX-WP-STATE-ROTATION-PORT-ADR0066`, commit `b53019ff`, and
  `PHX-WP-STATE-ROTATION-EXTRACT-AND-BUILD`, commit `c898536b`) extracted 10
  durable rules from earlier zones of `docs/state.md` into
  `guardrails/security.md` (SEC-10), `guardrails/quality-gates.md`
  (QG-08/QG-09), and `roles/elephant.md` (EL-01 addendum, two EL-09 addenda,
  two EL-22 addenda) and did a genuinely exhaustive sequential read of only
  one zone, lines 13387–13686, plus a comprehensive multi-pattern grep sweep
  across the rest of the (then 19,072-line) file. This round
  (`PHX-WP-STATE-ARCHIVE-COMPLETE`, 2026-08-19) did a full sequential
  Read-tool pass over the entire remainder never before read
  line-by-line — lines 13686–19155, the "Pipeline general/Nova-Cyborg-release
  history" block through every dated "Nova ..." section down to "Open items
  and next block" — and independently re-verified the prior rounds' grep
  sweep is still valid against the file's current content. Result: **no new
  durable/standing rule was found requiring extraction.** Every
  rule-shaped statement encountered in this range (e.g. "no 'X cannot happen
  because Y' without a test or measured probe", the explicit-pathspec/
  concurrent-commit-race discipline, the ban on unverified
  history-altering self-correction by a dispatched agent) was already
  captured verbatim or in substance by the two prior rounds' extractions
  (respectively: QG-12; EL-22's Revision paragraphs; and the standing
  Goldfish-briefing stop condition on `git reset`/history-altering
  self-correction). This is the honest discharge of ADR-0064 Decision 6's
  "no rotation while extraction is partial" caveat for exactly this range —
  see the ADR-0064 addendum this same round adds.

## Table of contents

Original section header text and its original line number in
`docs/state.md` immediately before this rotation (for navigability only —
line numbers do not apply to this archive file's own body below, which
starts fresh after this table):

- line 4977 (original `docs/state.md` numbering, pre-rotation): RESTART CHECKPOINT — 2026-08-08, WSL reboot + plugin refresh (READ THIS FIRST)
- line 13470 (original `docs/state.md` numbering, pre-rotation): Current status
- line 13862 (original `docs/state.md` numbering, pre-rotation): Phoenix branch history (sprint_phoenix, pre-merge, HEAD side)
- line 13885 (original `docs/state.md` numbering, pre-rotation): Operational head
- line 14714 (original `docs/state.md` numbering, pre-rotation): Design evidence
- line 14735 (original `docs/state.md` numbering, pre-rotation): Open items and next block
- line 14737 (original `docs/state.md` numbering, pre-rotation): 0.4.7 Elephant hotfix handover — Result-Authority bootstrap
- line 14741 (original `docs/state.md` numbering, pre-rotation): Pipeline general/Nova-Cyborg-release history (origin/main `2740041`, v0.5.3, theirs side)
- line 14763 (original `docs/state.md` numbering, pre-rotation): 2026-08-07 Nova GMW — Guard Maintenance Window: signed, time-boxed PO lift for GS-6/TP-*
- line 15083 (original `docs/state.md` numbering, pre-rotation): 2026-08-07 Nova HGO-Sig — signed admission path for Human Guard Override, everything GMW doesn't cover (current)
- line 15919 (original `docs/state.md` numbering, pre-rotation): 2026-08-07 Nova VII — first Nova A completion wave: 6 issues evidenced
- line 16027 (original `docs/state.md` numbering, pre-rotation): 2026-08-07 Nova VI — Nova A entry gate cleared, 10-issue status reconciled
- line 16089 (original `docs/state.md` numbering, pre-rotation): 2026-08-07 Nova V — backlog triage for 2026-08-05 through 2026-08-07
- line 16111 (original `docs/state.md` numbering, pre-rotation): 2026-08-07 Nova IV — 0.5.2 main-release signing, process friction recorded
- line 16169 (original `docs/state.md` numbering, pre-rotation): 2026-08-06 Nova III (night) — push executed, autonomous AFK prep
- line 16288 (original `docs/state.md` numbering, pre-rotation): 2026-08-06 Nova II (evening) — the guards that were never running
- line 17178 (original `docs/state.md` numbering, pre-rotation): 2026-08-06 Nova (afternoon) — authority-tier drift found and closed, ADR-0054 step 1, ADR-0055
- line 17481 (original `docs/state.md` numbering, pre-rotation): 2026-08-06 Nova — autonomous overnight session, marketplace-rename remediation, T1 Critic FAIL with three findings fixed
- line 17786 (original `docs/state.md` numbering, pre-rotation): 2026-08-05 Nova — preflight runner-identity fix, Claude local-dev doc, marketplace-collision finding
- line 17913 (original `docs/state.md` numbering, pre-rotation): 2026-08-04 Nova — Claude-session runner-routing fix + ADR-0051
- line 18200 (original `docs/state.md` numbering, pre-rotation): 2026-08-01 Nova — handover-only session cut
- line 18231 (original `docs/state.md` numbering, pre-rotation): 2026-07-31 PO session authorization — temporary protected-test lifts
- line 18242 (original `docs/state.md` numbering, pre-rotation): 2026-08-01 PO Sprint Nova authorization — standing bounded protected-test lifts
- line 18253 (original `docs/state.md` numbering, pre-rotation): 2026-08-01 Nova restart checkpoint
- line 18273 (original `docs/state.md` numbering, pre-rotation): 2026-08-01 Nova guard and local-plugin checkpoint
- line 18298 (original `docs/state.md` numbering, pre-rotation): 2026-07-31 0.4.7 release qualification — authoritative latest
- line 18317 (original `docs/state.md` numbering, pre-rotation): 2026-07-30 code-first 0.4.7 checkpoint — authoritative latest
- line 18360 (original `docs/state.md` numbering, pre-rotation): Operational head
- line 18576 (original `docs/state.md` numbering, pre-rotation): Open items and next block

## Archived content (verbatim, byte-for-byte, starts at original line 4977)
...
### THE EPIC IS MEASURED END TO END: 78 OF 157 CRITERIA, 45 OF 105 LIVE ISSUE BULLETS, 0 OF 8 ISSUES CLOSEABLE (2026-08-09)

The PO asked for evidence per acceptance criterion and a reconciliation against the
`sprint:phoenix` issues. Both now exist as **one committed, regenerable artifact**:
`specs/sprint-phoenix-epic/evidence/acceptance-evidence-map-20260809.md` (`8e7a2f7`),
generated by `acceptance-evidence-map.mjs` beside it.

**Read the generator, not the prose, if you doubt a number.** The verdict map, the
per-criterion evidence pointer and the 105-row bullet-to-criterion map are data in that
file; the document is its output. The next measurement regenerates it. That is deliberate
— the 2026-08-08 coverage run's numbers were already being quoted second-hand in this
handover within a day, and a hand-edited table is exactly the artifact that goes stale
while still reading as authoritative.

**What moved, and it is more than I expected.** Two fresh-context dispatches
(`PHX-FIN-A`, `PHX-FIN-B`, both read-only, both adjudicating from the tree and explicitly
forbidden to use this file as an evidence source) re-measured the 23 criteria that product
commits had touched since the baseline. Seven promoted to `implemented`:

- **PX0-AC-02** — the continuity-authority revision writer landed (`c62a3c4`) and its
  staged cases were registered into the canonical suite (`0f3b4c9`). The four criteria
  that were `designed-only` because they shared one absent callee — PX0-AC-03, -05, -06,
  -07 — are now `partial`, each with one named unpinned clause rather than no carrier.
- **PX0-AC-09/10/11/16/17** — these five were pinned only by a red, unregistered suite.
  Retiring it (`14cca32`) and replacing it with the registered
  `bootstrap-source-attestation-acceptance-tests` (`f32e21c`, `verify.mjs:333`) is what
  promoted them. **I verified the registration line myself rather than taking the report.**
- **A-AC-11** — the sharpest reversal. The baseline's headline finding was that the
  journal schema "models a different thing": a claim lifecycle where the criterion demands
  seven epistemic states. `5d0fc6a` added `assumptionState` with exactly those seven
  values, verbatim. **Checked at source** (`agent-decision-event.schema.json:14`).

71 implemented became **78 of 157**. The other twelve rows were confirmed unchanged.

**The gating slice did not move, and this is the sentence that matters for planning.**
P-AC-08's `feature-package` command family and its absent-manifest `draft` bootstrap
preview are built, tested and registered. Its **existing-manifest reconciliation and the
Result-reconciliation fence are still entirely absent** — and that is the half the
criterion makes the precondition for PHX-0's ruleset slice and for PHX-1. The slice is
half done in a way that reads as done from the commit log and is not.

**The number nobody had measured: the issues.** Applying the closure rule the package
already carries (`design/issue-coverage.md:201-204`) to all 105 live acceptance bullets:
**45 carried, 60 blocked by 48 distinct criteria, 0 of 8 issues closeable.** Per issue —
#5 3/6, #9 2/11, #17 3/6, #23 12/16, #24 7/12, #30 6/17, #31 4/17, #32 9/20. The blocking
set is concentrated, which is the useful part: **P-AC-11 alone blocks four bullets**,
K-AC-05 and P-AC-06 three each, and ten criteria block two or more. Closing those ten
would clear roughly a third of the blocked bullets without touching a new feature.

**EPIC-AC-04, gate by gate, because "almost done" is not a state.** Full Verify (368/368,
exit 0, exact binding on `3387065`), blocking Security (`blocking: false`, sast + secrets
pass) and the exact push/readback have **passed**. Privacy review, an independent
high-risk Critic **on the integrated candidate**, and explicit PO acceptance are
**absent**; the focused package check is partial for the P-AC-08 reason above. EPIC-AC-05
therefore still bites and the artifact says so in its own first section rather than
leaving a reader to derive it from a table.

**Two epic rows cannot be closed by writing code**, and they should not be planned as if
they could: EPIC-AC-03's recorded Spec §7 inventory deviation (now *executable* for the
first time, since its sanctioned route was the revision writer that just landed), and
H-AC-11's GMW half, which its own PO amendment records as a **proved impossibility** for
Increment 1 rather than unfinished work.

**One thing I did not do, and the reason is a gate not a preference.** The run stops
before layer 3 of `docs/push-release-flow.md`. `8e7a2f7` and the reconciliation record
