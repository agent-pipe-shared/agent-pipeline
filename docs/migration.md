...
`AGENTS.md` is not generated by setup and is not required for a project. If an
older pipeline-owned adapter is present, run the separate command
`node setup.mjs --migrate-agents-adapter`. It recognizes only the fixed public
legacy metadata and integrity proof; it never reads, copies, logs, or overwrites
unknown user-owned content. Unknown, unreadable, or ambiguous presence stops at
a manual/PO migration gate without a write. Absence stays absent.

The command writes only the recognized legacy adapter, and only after validating
the runtime manifest, `pipeline-core:pipeline-start`, and the Operating Model.
The resulting pointer is intentionally thin: for Codex and other non-Claude
runtimes it states a methodology-only entry, not hook equivalence, tool/agent
integration, model binding, or global host enforcement.
...
## Optionale AGENTS-Adapter-Migration

`AGENTS.md` wird nicht durch Setup erzeugt und ist für ein Projekt nicht
erforderlich. Ist ein älterer pipeline-eigener Adapter vorhanden, nutze den
separaten Befehl `node setup.mjs --migrate-agents-adapter`. Er erkennt nur die
fest definierte öffentliche Alt-Metadaten-/Integritätsform; unbekannte
nutzergeführte Inhalte werden weder gelesen, kopiert, protokolliert noch
überschrieben. Unbekannte, nicht lesbare oder mehrdeutige Präsenz stoppt ohne
Write am manuellen/PO-Migrationsgate. Abwesenheit bleibt Abwesenheit.

Der Befehl schreibt ausschließlich den eindeutig erkannten Altadapter und erst
nach Prüfung von Runtime-Manifest, `pipeline-core:pipeline-start` und Operating
Model. Der resultierende Pointer ist absichtlich dünn: Für Codex und andere
Nicht-Claude-Runtimes beschreibt er nur den methodischen Einstieg, keine
Hook-Äquivalenz, Tool-/Agenten-Integration, Modellbindung oder globale
Host-Durchsetzung.
