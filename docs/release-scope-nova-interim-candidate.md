...
- Enumerated candidate commits (in the order named by this dispatch's
  briefing): `0d63a3f2, f3ac7cfd, 6a1a5514, d4d8843d, ef0ec784, 1a757618,
  2a4968cc, 89a07b2c, 57821c91, 0aaeb881, e9bd4a23, 0e9f82fb, 3475322b`.

The candidate's **binding** commit and tree — the exact object identities a
release preflight artifact fixes and a signature is later computed over —
...
Machine output of `node plugins/pipeline-core/scripts/dispatch-authorship-verify.mjs --range 1b467f98..HEAD`, run by this dispatch:

```
FAIL         41967fd3be76 record-not-terminal: record outcome `in-progress` is not terminal
...
`41967fd3` is this dispatch's own (`NVA-CFIX-1`) piece-2 commit; its `FAIL
record-not-terminal` is expected and transient — `evidence/dispatch-record-
NVA-CFIX-1.json` still read `outcome: "in-progress"` at the moment this
command ran, ahead of this dispatch's own closing act.

The four commits classified `elephant-direct-undeclared`
(`0d63a3f2`, `6a1a5514`, `d4d8843d`, `0aaeb881`) plus `e9bd4a23` are
Elephant-direct commits that predate the adoption of the `Dispatch: stage-0
(elephant)` trailer in this branch. Their history cannot be corrected —
rewriting history is forbidden by this repo's hard rules — so they stay
