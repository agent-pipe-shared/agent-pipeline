...
| A local project imitates Codex-owned controls so onboarding skips Git and `.codex/**` writes. | The host-managed path recognizes only an otherwise fresh root with empty, non-symlink, non-writable `.git` and `.codex` directories and an optional directory meeting the same conditions at `.agents`. It never chmods, writes, or replaces those controls; an explicit activation writes only `pipeline.user.yaml` and `.claude/**`, then the readback accepts the special projection only when calibration explicitly records `repositoryMode: "host-managed"` and every pending target is below `.codex/`. Every near-match remains rejected or follows the ordinary onboarding path. | Filesystem permissions are host observations, not cryptographic host identity. A local actor who can construct this exact inert layout can cause an explicitly activated setup to omit local Git initialization, but cannot cause reserved-path writes or obtain a normal runtime-noop claim. Operators must select the intended workspace and configure project verification before delivery. |
