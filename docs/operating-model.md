...
| Rule | Why | Verification method |
|---|---|---|
| **Must not:** The Elephant writes no production code. | Role split: execution needs a fresh context; the Elephant stays lean and unbiased for the gate decision (confirmation bias). | Production diffs map to a Goldfish dispatch record and grounded `Dispatch:` trailer; `AI-Assisted: true` records anonymous assistance only. |
...
- **Three-artifact archive rule:** for every larger task, three things are permanently versioned and archived: (1) the problem description/spec, (2) the acceptance criteria, (3) the result/completion report. **Full chat logs are NOT archived** ("mostly token noise"); `AI-Assisted: true` is only an anonymous assistance signal, while the dispatch record and grounded `Dispatch:` trailer provide work-package provenance. Provider/model/session/account correlation is prohibited in commit metadata. **Why:** a searchable archive of one's own judgment compounds; chat logs don't. **Verification method:** the close ritual checks that the three artifacts are filed and metadata remains anonymous, from rigor level 1 up.
...
| Regel | Warum | Prüfweise |
|---|---|---|
| **Verbot:** Der Elephant schreibt keinen Produktions-Code. | Rollenschnitt: Ausführung braucht frischen Kontext; der Elephant bleibt schlank und unvoreingenommen für den Gate-Entscheid (Selbstbestätigungs-Bias). | Produktions-Diffs sind einem Goldfish-Dispatch-Record und geerdeten `Dispatch:`-Trailer zugeordnet; `AI-Assisted: true` belegt nur anonyme Assistenz. |
...
| 3b | PO-Gate: PRD-Freigabe | geprüfte Spec/Plan + `prd_<topic>.md` in der aus `.claude/pipeline.yaml` gelesenen `language.human_facing` | das `freigegeben` des PO oder Änderungsauftrag | PO | **Pflicht bei Rigor ≥1 ODER Klasse hoch; echter Stufe-0-Fast-Path entfällt.** Fehlt die kompilierte Sprache, stoppt der Autorierungspfad zur Setup-Korrektur. Das lesbar vorgelegte PRD mit Status `freigegeben` ist die einzige Autorität für genau einen ersten Implementierungs-Dispatch; keine zweite Implementierungsfreigabe. Merge-, Push- und Release-Gates bleiben getrennt. |
...
**Schritt 3b (PO-Gate: PRD-Freigabe) — Detail:** Nach bestandenem Readiness-Check und VOR dem ersten Implementierungs-Dispatch führt der Elephant `node harness/scripts/check-po-language-projection.mjs` aus und liest dann `language.human_facing` aus der kompilierten `.claude/pipeline.yaml`. Fehlt, ist sie unlesbar, ungültig oder widerspricht sie der validierten Quelle, stoppt er und lässt `pipeline.user.yaml`/`node setup.mjs` korrigieren; es gibt keinen Sprachdefault. Danach legt er ein `prd_<topic>.md` in dieser Sprache vor — Produkt-Rationale (Was/Warum/Scope/Nicht-Ziele/Risiken/betrachtete Alternativen), NICHT die Akzeptanzkriterien (die stehen agent-facing englisch in der Spec; PRD und Spec referenzieren einander, keine Doppelung). Ablage: `specs/<task>/prd_<topic>.md` (Drei-Artefakte-Paket mit Spec + Readiness + Review). **Pflicht bei Rigor ≥1 ODER Klasse hoch; ein echter Stufe-0-Fast-Path (§3.3) entfällt** — kleine Hotfixe brauchen kein Produkt-Review. Freigabe per EL-17a (nummerierte Inline-Zusammenfassung + Datei-Verweis); ein lesbar geliefertes PRD mit Status `freigegeben` ist das Gate und autorisiert genau einen ersten Implementierungs-Dispatch, ohne zweite Implementierungsfreigabe. Merge, Push und Release bleiben eigene Gates. **Optionales Begleitartefakt OHNE Gate:** `sdp_<topic>.md` (Software Development Plan) je Thema — dokumentiert, aber kein Pflicht-Bestätigungspunkt (Enterprise-Vorbehalt, Wiedervorlage bei Bedarf). Nach dem `freigegeben` des PO wird die Freigabe zusätzlich deterministisch verbucht via `node harness/scripts/pipeline-state.mjs approve-plan --by po` (Grundlage des Dev-Plan-Gate-Hooks).

**PRD-Sprachvorgabe:** Verbindlich für jedes künftige PRD — Zielleser ist der PO, nicht der Agent — die konfigurierte, kompilierte `language.human_facing`, Klartext, kurze Sätze; das PRD ist ein Entscheidungsdokument, kein Arbeitsauftrag. Fehlt diese Runtime-Autorität, stoppt der Pfad zur Setup-Korrektur statt eine Sprache zu erfinden. Jeder Block beantwortet drei Fragen: Welches Problem? Was ändern wir? Was hast du davon? Regel-IDs, Dateipfade und Fachjargon gehören NICHT in den Haupttext — kompakte Technik-Zeilen oder ein Anhang tragen sie, die nachgelagerten Goldfisch-Briefings sowieso. Bei feedback-/review-getriebenen PRDs gehört eine Abdeckungs-Matrix dazu („dein Input → wo im PRD"), damit der PO Vollständigkeit prüfen statt vertrauen muss. Am Ende stehen nummerierte Entscheidungspunkte — was explizit zu entscheiden ist, getrennt von reiner Kenntnisnahme. Vorlage: [../templates/prd.md](../templates/prd.md); Elephant-Pflicht: `roles/elephant.md` EL-19.
...
- **Drei-Artefakte-Archiv-Regel:** Je größerer Aufgabe werden dauerhaft versioniert abgelegt: (1) Problembeschreibung/Spec, (2) Akzeptanzkriterien, (3) Ergebnis/Abschlussbericht. **Volle Chatlogs werden NICHT archiviert** („mostly token noise"); `AI-Assisted: true` ist nur ein anonymer Assistenzmarker, während Dispatch-Record und geerdeter `Dispatch:`-Trailer die Work-Package-Provenienz tragen. Provider-/Modell-/Session-/Account-Korrelation in Commit-Metadaten ist verboten. **Warum:** Ein durchsuchbares Archiv des eigenen Judgments kompoundiert; Chatlogs tun das nicht. **Prüfweise:** Close-Ritual prüft ab Rigor-Stufe 1 die Ablage der drei Artefakte und anonyme Metadaten.
- **Error-Register:** `backlog/error-register.md` ist die alleinige öffentliche konkrete Formautorität für ein gedeckeltes, kuratiertes Triage-Board; dieser Operating Model bleibt die Prozessautorität und Schritt 7b des `close-block` die Close-Anweisung. Das Board konsolidiert ähnliche Fehlerklassen, bleibt bei ungefähr 30 Klassen und wird nie als Briefing-Kontext geladen. Es enthält keine Rohereignisse, Chronologie, Counts, Rankings oder Prioritäten. Eine wiederholte Klasse erhält im selben Close eine Disposition nach Mechanismus > Template > kuratierte Lektion oder eine begründete Zurückstellung; eine bloße Wiederholungsmarkierung ist ungültig. Der maschinelle `check-error-register` prüft genau diese öffentliche Form fail-closed; es gibt keinen privaten Fallback oder eine zweite Registerform.
