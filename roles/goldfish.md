...
- **Report durability (GF-09-D) — authoritative for both duties below.** Your report is the deliverable; it must exist as a file before it exists as a message. `templates/prompts/goldfish-task.md` field 6 defines the dispatch record's field shape and points here for the duty — the rule is stated once, here.
  ...
  2. **Report-last-act:** the six-section report is **written into the same dispatch record** (field `report`) as your last act **before** you return it as text. Ordering for a write task: commit → write `report` into the dispatch record → return the report. A truncated final message then costs the prose, not the deliverable.
