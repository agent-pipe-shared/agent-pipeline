...
- **Check:** Dispatch references the readiness result; the Critic (rigor 2) flags implementations without a passed mandatory check. Details: `docs/operating-model.md` §3.4.

### EL-19 (MUST) — PO gate (PRD) before implementation

- **Rule:** For **rigor ≥ 1 OR risk class high**, after the spec passes its readiness check (EL-07) and BEFORE the first implementation dispatch (EL-05/EL-08), obtain the PO's PRD release. Produce a German `prd_<topic>.md` — product rationale (Was/Warum/Scope/Nicht-Ziele/Risiken/betrachtete Alternativen), NOT acceptance criteria (those stay agent-facing English in the spec, which the PRD references and does not duplicate). **PO-readable content:** the PRD content itself must be PO-readable per `docs/operating-model.md` §3.2 step 3b / `templates/prd.md` — German Klartext, each block answers Problem → Änderung → Nutzen, rule-IDs/file paths/jargon out of the main narrative (compact tech-lines or an appendix only), a coverage matrix for feedback-/review-driven PRDs, and numbered decision points at the end. **Sharpened:** PROACTIVELY deliver it as a READABLE document per EL-17(a) (numbered inline summary + file reference **+ readable delivery to the PO's device/render — remote sessions: send it; a repo path alone is NOT delivery**); explicitly state that you are waiting for the literal word „freigegeben", and dispatch NO implementation until it arrives. Omitted proactive delivery is a process incident, flagged at the close-/critic-trajectory checkpoint. A **true stage-0 fast-path** task (`docs/operating-model.md` §3.3) is exempt — small hotfixes need no product review. Location: `specs/<task>/prd_<topic>.md`. Optional, gate-free companion: `sdp_<topic>.md` (Software Development Plan) — documented, never a mandatory sign-off.
...
- **Rule:** Apply the trigger matrix (`docs/operating-model.md` §4.2) when a goldfish delivers: no Critic only at rigor 0 / low risk / no flag; the review-tier model / `max` standard; escalate to a higher-capability model at high risk class; escalate to a higher-capability model **plus `--bare` isolation** for EVERY architecture/guardrail/security diff regardless of size. Canonical trigger wording (authoritative; normative source: `docs/operating-model.md` §3.3/§4.2, ADR-0003, ADR-0014): every architecture/guardrail/security diff runs the Critic on a higher-capability model / `max` AND additionally in `--bare` isolation; rigor level 2 makes the Critic mandatory (default: the review-tier model / `max`); the higher-capability-model escalation applies there only when, additionally, the risk class is high OR an architecture/guardrail/security diff is present.
...
- **Check:** Handover session block carries the ledger; close-step telemetry lines and the 6b authorship check cite ledger entries, not recollection; outcome/effect claims without a named Wirkort are flagged at close/critic review as claim-hygiene violations.
...
- **Rule:** the PO-facing chat is limited to four event classes — **Befund** (finding) · **Entscheidungsbedarf/Gate** (decision needed / gate) · **Inzident/Stop** (incident/stop) · **Block-Ergebnis** (block result) — outcome first, compact, German. NO mechanical progress narration (dispatch announcements, interim ceremony). Visibility comes from the harness task display + the `state.md` dispatch ledger (EL-21), not chat prose. For runs longer than ~15 minutes without a qualifying event, post ONE liveness line.
...
- **Rule:** At every handover moment — package/wave boundary (critic PASS + commit/push done), PRD gate passed, or before the first dispatch of a new package — check context fill. At ≥ ~100k tokens you MUST present the PO a compact block: verbatim `/compact` plus a one-line focus hint naming what the next phase needs. Target window 100–150k; >150k counts as an overdue cut, named honestly, never hidden. `/compact` stays boundary-only — never mid-dispatch, never with open gates, never with unpersisted state; precondition: handover file/ledger current (persisted state = compact-safe, `docs/operating-model.md` §5.1). Pure-design-tier sessions (a PO-exception class, MP-01) prefer the harder cut at EVERY phase boundary — session cut (`/clear` + new session with `docs/state.md` as bridge) or `/compact`. When the advisor is enabled (`models.advisor`, MP-26), after every `/compact` actively verify advisor availability and note the outcome in the session block. `/compact` is a user command: if the PO is absent at a boundary, present the block, continue working, and re-offer at the next boundary — never block on it, never treat silence as refusal or approval.
...
- **Check:** Dispatch ledger / handover block shows the compact-block presentation at boundaries with fill ≥100k; advisor-enabled sessions log the post-compact advisor check; a boundary crossed silently past 150k without an offered block, or a stop notification with no independent state check, is a lifecycle-violation finding at close/critic review.
...
- **Why:** Incidents: the question UI swallowed a decision table ("oft klappen diese Rückfragen-UI-Dinger nicht" — the PO); a session-resume mix-up nearly cost orientation. These rules were adopted ad hoc in-session and as memory notes — they belong in the repo, because chat and memory evaporate (GL-07; memory is mirror only).
...
- **Recommend the cut, don't wait for it to happen to you:** at phase boundaries (design-gate → implementation wave → critic → close) actively RECOMMEND `/compact <fokus>` or a session cut to the PO — you cannot run `/compact` yourself; silence is the failure mode, not a neutral default. **Checkpoint duty (EL-25):** at ≥ ~100k context fill this recommendation becomes a MANDATORY compact-block presentation, not an option.
- **`/compact` only at task boundaries and only with a focus argument; topic switch = `/clear` + `/rename`.** One topic per session. Target checkpoint window: ~100–150k; >150k is an overdue cut, named honestly (EL-25).
