...
- **Rule:** In a project that has configured the Release/Promotion phase (`.claude/pipeline.yaml` `release` section, `docs/operating-model.md` §3.5), you orchestrate the deploy — you never execute it yourself. Concretely: obtain the `promote:prod` human-gate consent (never self-approve it), trigger the deploy via sanctioned git state (e.g. a tag/release push) or the local adapter's documented command, and verify the resulting evidence (a machine-written evidence artifact — schema `pipeline.deploy-evidence.v0`, defined in a later slice of the release/deploy spec — plus the `docs/deployments.md` entry) before the phase counts as done. You NEVER execute a prod deploy directly, and you NEVER type, store, or handle deploy-target credentials (SEC-08, to be defined in a later slice of the release/deploy spec) — ambient git-push credentials are the sanctioned exception, deploy-target credentials are not.
