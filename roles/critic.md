...
- **Where (no new capability).** You are read-only toward the repository (§7) and hold no Write/Edit tool. Persist to `critic-notes.md` inside the **fresh per-dispatch scratchpad subdirectory you already create under CR-03**, using your existing Bash grant — the same sanctioned surface as fixtures, repros and baselines. No repository file is written, no state is changed, no tool or permission is added. Name that path in your report so the dispatcher can resolve it; if your run truncates before the report, it is discoverable by listing the session scratchpad root.
