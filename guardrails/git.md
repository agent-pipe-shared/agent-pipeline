...
- Push/merge autonomy is whatever the calibration grants — and no more; consent for outward-effective actions never carries across contexts (`guardrails/global.md` GL-04). In this repo since 2026-08-06 (PO decision, superseding ADR-0017's standing approval): the push gate is `approval: required` — every push needs an approval recorded against the exact candidate commit, plus an Ed25519 human proof unless an explicit waiver stands the proof (never the approval) down ([ADR-0055](../docs/adr/0055-critical-human-proof-waiver.md)). Destructive push forms remain guard-blocked regardless.
