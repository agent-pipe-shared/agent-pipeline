...
`1f9f7648c6e925efda8bdfbaf7136afd100d1e21`, tree
`d359c1e6b50b3aa4c5228dda3b2e7df07f79c7b4`. It contains the CI-fixture
repair, the first-reader README capability list, and the small push-prepare
evidence-location correction. It is intentionally separate from subsequent
Nova backlog work and has no pending local source changes.

- **Prerequisite, no PO decision:** run the calibrated Push Verify from the
  candidate worktree in an attended terminal. The Codex-restricted sandbox
  produced a known false red in an Antigravity fixture through `spawnSync git
  EPERM`; the identical fixture passed 57/57 outside that sandbox. The
  Consumer-safe-path regression found in the same run was corrected and its
  focused suite passed 9/9.
- **PO action after a green candidate-bound Verify:** perform exactly one
  fresh `push`-kind authorization for `upstream` to `refs/heads/main`. Its
  subject digest must be generated by `push-prepare.mjs` after Verify is green;
  no earlier proof or signature is reusable after the candidate changed.
- **Not authorized by this note:** a force push, tag mutation, publication,
  release creation, plugin installation, or any second signature. Those
  remain separate external actions.
- **Blocking status:** blocks only the main push of this candidate. It does
  not block local Nova backlog work, PO-list maintenance, or ordinary commits.
