...
## Selected requirement — PO decision 2026-09-12

When and only when the exact child dispatch is machine-bound to the canonical
Advisor prohibition, the raw Advisor call is denied before model effect and a
minimal private audit event is recorded. Without that prohibition binding,
this rule does not block the call; MP-26's existing demand, consent, candidate,
route, evidence, reuse and receipt checks remain mandatory. The policy core is
runner-neutral, while a runner adapter may claim enforcement only where the
host supplies an observed identity chain that binds the exact child.
