...
| P1 | The pre-push hook must not remain stale after the plugin cache updates. | Installer detects upgrades, but currently relies on a visible installation action. | Design an explicit, consent-preserving self-update/reinstall path; do not silently overwrite hooks. |
