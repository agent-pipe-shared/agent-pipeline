...
| P1 | The pre-commit backstop must provide testable, actionable refusal diagnostics. | Confirmed local Nova-B defect: its registered suite sees correct refusal exit codes but empty child diagnostics in all ten negative end-to-end cases. | Diagnose the Node/Git process boundary and require a freshly executed green suite receipt; do not weaken the diagnostic assertions. |
