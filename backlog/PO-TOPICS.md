...
| P1 | The Greenfield recovery test must be registered in the protected full-Verify manifest. | Candidate Verify correctly found one missing entry for `pipeline-state-late-verify.test.mjs`. The precise one-line registration is blocked by protected-path rule `TP-3`; no edit, staging, override, or source rewrite was attempted. | Decide whether to issue a fresh, narrowly bound signed override for that one manifest entry, or keep this as input to the protected-test-path governance simplification work. |
