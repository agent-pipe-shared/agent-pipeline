...
### Shipped default (preset, `pipeline.user.yaml` → `models.*`)

The pipeline distinguishes five role tiers. `setup.mjs` writes a preset matching the PO's subscription tier at setup time; the **Max preset** below is the recommended starting point and this repo's committed default state:
...
| `models.advisor`    | optional second-opinion advisor    | — (**disabled** by default) | — |

All values are overridable in `pipeline.user.yaml` — the "Pro" preset, for instance, runs every role on the cheaper model with staggered effort (see comment in `pipeline.user.yaml`). From here on this policy speaks only of **tiers**, never product names. **`goldfish-deep` is NOT its own config tier:** it is Implement-tier work with a full thinking budget → runs on the Implement-tier model at effort `xhigh` (on the Design-tier model where genuine design latitude exists).
...
- **Must:**
  (a) The advisor is used only when `models.advisor.enabled: true` is configured (or via a dated project exception) — default is OFF (see preset, Section 1).
