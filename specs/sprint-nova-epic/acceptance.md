...
| NVA-G15 | A durable authorization for push, human-gated deploy or publication SHALL require an exact externally verified human proof bound to the clean candidate, bound PRD/Spec, closed action kind and writer-owned action subject. Chat approval or bare attribution alone (a recorded approval with no verified signature) SHALL fail closed at those boundaries. This proof MAY be verified against any well-formed key when no trust-anchor set is configured — a deliberate "a human signed, not which pre-registered human did" posture (`specs/sprint-nova-epic/plans/nova-setup-bootstrap.md` §5a) — with the signing key's identity still captured in the approval record so attribution is preserved though not restricted; a populated trust-anchor set additionally enforces membership. | Positive/negative critical-proof and state-writer tests, including candidate/subject/kind/expiry/replay cases, plus absent/populated trust-anchor coverage. |
| NVA-G16 | A remote provisional code SHALL be one-time, 30-minute-or-shorter, candidate-and-scope-bound and usable only for declared local continuation. It SHALL be rejected by push, deploy, publication, release, override, merge and deletion paths. | Provisional-receipt one-time/expiry/scope tests plus final-gate rejection tests. |
| NVA-G17 | Each Cyborg handover item SHALL retain `{ itemId, spec, candidateCommit, evidence }` on Nova's selected candidate; only a current passing row may move through the canonical writer, with post-apply readback. | Six named-suite receipts, tuple records, reconciliation preview/apply/readback or a narrow open follow-up. |
