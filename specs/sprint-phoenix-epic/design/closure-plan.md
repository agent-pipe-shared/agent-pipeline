...
| H-AC-12 | partial | WP-PO | the shared dual-evaluation primitive exists and closes 2 of ~6 named readers (guard-devplan.mjs, change-control.mjs). UPDATE 2026-08-17 (PHX-WP-POAMEND, commit `e9054995`): the 2026-08-11 PO decision for release-planning/deploy-consumption now landed as an acceptance.md amendment — closes those 2 readers formally, bringing the closed set to 4 of ~6. UPDATE 2026-08-17 (PHX-WP-HAC12, commit `ae13b68b`, independently re-verified): guard-push.mjs and pipeline-state.mjs now wired with the same opt-in, fail-closed, MIGRATION_COMPAT-tracked pattern — but only to the same weaker "structural self-consistency" depth change-control.mjs already had, not to Git-guard override consumption's actual canonical-ledger depth (`governance-authority.mjs`); currently inert since nothing yet writes a decisionReference for push/deploy. Stays open: Git-guard override consumption (the one reader where the real mechanism is already available) remains fully untouched, and whether the migration dual-evaluation/shared-owner/expiry clause is migration-conditional or standing for these two readers is a genuine open interpretive question, not a code task |
