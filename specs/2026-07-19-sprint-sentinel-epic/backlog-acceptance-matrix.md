...
| `pipeline.t1-governance-path-preflight` — **open**, partial | Governance-path clause is proven closed (`CPG01/CPG02/CPG05/CPG06`, `CPP01` green, no code changed); the AC's stale gate-ETA clause is retired (2026-08-18 PO decision, reworded rather than adding a `gateEta` field); tool-setup disposition remains incomplete. | Critic packet governance, workflow preflight/writer modules and tests. | Specify, register, and prove the remaining T1 tool-setup AC, then close. |
