...
- **Push policy:** the sole possible remote-action exception is an exact, securely resolved **PHX-2 Human Governance Decision Ledger** PO decision, bound by its **Authority Resolver** to one action, remote, ref, candidate and work package. Until that PHX-2 Ledger/Resolver path exists and produces that proof, this Pipeline has no executable remote-action exception: every push or other remote action remains an explicit PO gate, but a PO direction is only proposed Ledger input, never executable authority. It must not be promoted, copied or inferred from mutable `pushApproval` or other state, CLI input, Git history or metadata, cache, journal, lifecycle/readback record, agent record, runner or AGY value. What stays forbidden is enforced by the guard union, not by asking: never force-push, never rewrite history, never delete protected branches/tags, never skip hooks. Project repos keep their own calibration rules (GIT-05).
