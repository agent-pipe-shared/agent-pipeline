...
- **Push policy (revised 2026-08-06, PO decision — supersedes the standing approval [ADR-0017] recorded for this repo):** the push gate is `approval: required`. Every push needs an approval recorded against the exact candidate commit (`pipeline-state.mjs approve-push`). **How a human clears it is one setting**, `gates.push_approval` in `pipeline.user.yaml` ([ADR-0056](docs/adr/0056-push-approval-mode.md)): `signature` (default, and this repo's value) demands a detached Ed25519 proof with the private key outside the repository; `chat` lets the human clear it in-session, still commit-bound and still labelled in the record, but an attribution record rather than proof. Anything unreadable or unrecognised resolves to `signature`. What stays forbidden is enforced by the guard union, not by asking: never force-push, never rewrite history, never delete protected branches/tags, never skip hooks. Project repos keep their own calibration rules (GIT-05).
