...
- **Push policy:** the sole possible remote-action exception is an exact, securely resolved **PHX-2 Human Governance Decision Ledger** PO decision, bound by its **Authority Resolver** to one action, remote, ref, candidate and work package. Until that PHX-2 Ledger/Resolver path exists and produces that proof, every push or other remote action requires an explicit PO gate. What stays forbidden is enforced by the guard union, not by asking: never force-push, never rewrite history, never delete protected branches/tags, never skip hooks. Project repos keep their own calibration rules (GIT-05).
