...
- **Self-application push policy:** while `publicPushIdentity.mode: required`, only its exact calibrated feature branch may be pushed after the required exact-commit gates. `main`, tags, releases, merges, force-pushes, history rewrites, protected-branch/tag deletions and hook skips remain forbidden. A historical standing-approval record does not authorize a different ref; project repos follow their own calibration rules (GIT-05).
