...
- **Push policy (revised 2026-08-06, PO decision — supersedes the standing approval [ADR-0017] recorded for this repo):** the push gate is `approval: required`. A push needs an approval recorded against the exact candidate commit (`pipeline-state.mjs approve-push`), and by default an Ed25519 human proof with the private key outside the repository. Raw `git push` fails closed until that exists. The proof — not the approval — may be stood down by an explicit, reasoned waiver in `project/critical-human-proof.json` ([ADR-0055](docs/adr/0055-critical-human-proof-waiver.md)); this repository ships none. What stays forbidden is enforced by the guard union, not by asking: never force-push, never rewrite history, never delete protected branches/tags, never skip hooks. Project repos keep their own calibration rules (GIT-05).
